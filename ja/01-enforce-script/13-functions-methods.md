...
[Home](../../README.md) | [<< 前: Gotchas](12-gotchas.md) | **Functions & Methods**

---

## はじめに

関数はEnforce Scriptにおける動作の基本単位です in Enforce Script. Modが実行するすべてのアクション --- spawning an item, checking a player's health, sending an RPC, drawing a UI element --- は関数の中にあります. 宣言方法を理解すること, pass data in and out, and work with エンジン's special modifiers is essential for writing correct DayZ mods.
...
すべての関数には a 戻り値の型, a name, and a パラメータリスト. 本体は波括弧で囲まれます.
...
Standalone (global) functions exist outside any class. They are rare DayZ Moddingにおいて --- nearly all code lives inside classes --- but に遭遇するでしょう a few in the vanilla scripts.
...
の大部分は functions DayZ Modにおいて are instance methods --- they belong to a class and operate on that instance's data.
...
Instance methods have implicit access to `this` --- a reference to the current object. めったに必要ありません write `this.` explicitly, but it can help disambiguate when a parameter has a similar name.
...
Enforce Script supports four parameter passing modes. Understanding them is critical because the wrong mode leads to silent bugs where data never reaches 呼び出し元.

### 値渡し（デフォルト）

修飾子が指定されていない場合, the parameter is passed **by value**. For primitives (`int`, `float`, `bool`, `string`, `vector`), コピーが作成されます. 関数内の変更は影響しません 呼び出し元's variable.
...
クラス型の場合 (objects), by-value passing still passes a **reference to the object** --- but the reference itself is copied. You can modify the object's fields, but you cannot reassign the reference to point to a different object.
...
The `out` keyword marks a parameter as **output-only**. The function writes a value into it, and 呼び出し元が受け取る that value. The initial value of the parameter is undefined --- do not read it before writing.
...
バニラスクリプトでは `out` extensively for engine-to-script data flow:
...
The `inout` keyword marks a parameter that is **both read and written** by 関数. The caller's value is available inside 関数, and any modifications are visible to 呼び出し元 afterward.
...
The `notnull` keyword tells コンパイラ (and エンジン) that the parameter must not be `null`. If a null value is passed, the game will crash with an error rather than silently proceeding with invalid data.
...
Functions return a single value. The 戻り値の型 is declared before 関数 name.
...
When a function returns an object, it returns a **reference** (コピーではありません). The caller receives a pointer to the same object in memory.
...
よくある罠: `JsonFileLoader<T>.JsonLoadFile()` returns `void`, not the loaded object. You must pass a pre-created object as a `ref` parameter.
...
バニラスクリプトでは default parameters extensively:
...
Proto native methods are declared in script but **implemented in the C++ engine**. They form the bridge between your Enforce Script code and the DayZ game engine. You call them like normal methods, but その実装を見たり変更したりすることはできません.
...
| `proto native owned` | Returns a value 呼び出し元 owns (manages memory) | `proto native owned string GetType();` |
...
最も一般的な修飾子. These are straightforward engine calls.
...
The `owned` modifier means the return value is allocated by エンジン and **ownership is transferred to the script**. This is primarily used for `string` returns, where エンジン creates a new string that the script's garbage collector must later free.
...
The `external` modifier indicates 関数 is defined in a different script module. This allows cross-module method declarations.
...
The `volatile` modifier tells コンパイラ 関数 may have **side effects** or may **call back into script** (creating re-entrancy). The compiler must preserve full context when calling these.
...
### Proto Nativeメソッドの呼び出し

他のメソッドと同様に呼び出します. 重要なルール: **決して試みないでください override or redefine a proto native method**. They are fixed engine bindings.
...
Use static methods when 関数 does not need any instance data:
...
Use instance methods when 関数 needs access to the object's state:
...
When a 子クラス needs to change the behavior of a parent method, it uses the `override` keyword.
...
2. **Signature 正確に一致する必要があります** --- same 戻り値の型, same parameter types, same parameter count.

3. **`super.MethodName()` calls the parent** --- use this to extend behavior rather than completely replace it.

4. **Private methods オーバーライドできません** --- they are invisible to 子クラスes.

5. **Protected methods オーバーライドできます** --- 子クラスes see and can override them.
...
省略すると `override`, コンパイラ may emit a warning but will **not** error. Your method silently becomes a new method instead of replacing the parent's. The parent's version runs whenever the object is referenced through a parent-type variable.
...
**Enforce Script does not support method overloading.** You cannot have two methods with the same name but different パラメータリストs. Attempting this コンパイルエラーが発生します.
...
You typically `override` event methods in 子クラスes rather than defining them from scratch:
...
The key takeaway: `event` is a declaration modifier, not something you invoke. エンジンは呼び出します event methods at the appropriate time.
...
The `thread` keyword goes on the **call**, not 関数 declaration. The function itself is a normal function --- what makes it a coroutine is how you invoke it.
...
The `owner` is the object that started the thread (or `null` for global threads). The `name` is 関数 name.

### 使用するタイミング Threads (and When Not To)
...
| Params | variadic | Parameters to pass to 関数 |

### 呼び出しカテゴリ

| カテゴリ | 目的 |
|----------|---------|
| `CALL_CATEGORY_SYSTEM` | General-purpose, runs 毎フレーム |
...
3. **Prefer out parameters over complex 戻り値の型s** --- when a function needs to communicate success/failure plus data, use a `bool` return with `out` parameters.
...
5. **Document proto native limitations** --- when wrapping a proto native call, note in comments what エンジン function can and cannot do.
...
| Guard clause `if (!GetGame()) return` at method start | COT Admin Tools | Every method that touches エンジン starts with null checks to avoid crashes during shutdown |
...
| `override` is optional | Could be inferred | Omitting it silently creates a new method instead of overriding; 常に含めてください |
| デフォルト parameter expressions | Should support function calls | Only literal values (`42`, `true`, `null`, `""`) are allowed; no expressions |
...
Enforce Script does not support overloading. Two methods with the same name cause a コンパイルエラー.
...
Some functions (notably `JsonFileLoader.JsonLoadFile`) return `void`. Trying to assign their result causes a コンパイルエラー.
...
| デフォルト value | `void Fn(int x = 5)` | Literals only, no expressions |
