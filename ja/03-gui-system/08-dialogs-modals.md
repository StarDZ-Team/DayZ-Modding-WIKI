...
[Home](../../README.md) | [<< 前: Styles & Fonts](07-styles-fonts.md) | **Dialogs & Modals**

Dialogs are temporary overlay windows that demand user interaction -- confirmation prompts, alert messages, input forms, and settings panels. この章では the built-in dialog system, manual dialog patterns, layout structure, focus management, and common pitfalls.
...
- **Modal** -- Blocks all interaction with content behind the dialog. The user must respond (confirm, cancel, close) before doing anything else. 例： quit confirmation, delete warning, rename prompt.
- **Modeless** -- Allows the user to interact with content behind the dialog while it remains open. 例： info panels, settings windows, tool palettes.
...
`UIScriptedMenu` is エンジン-level 基底クラス for all menu screens in DayZ. It integrates with the `UIManager` menu stack, handles input locking automatically, and provides lifecycle hooks. Vanilla DayZ uses it for the in-game menu, logout dialog, respawn dialog, options menu, and many others.
...
`ShowScriptedMenu()` pushes the menu onto エンジン menu stack, triggers `Init()`, then `OnShow()`. `Close()` triggers `OnHide()`, pops it from the stack, and destroys the widget tree.

### 主要なライフサイクルメソッド

| Method | 呼び出しタイミング | 一般的な使用法 |
|--------|------------|-------------|
...
Because `UIScriptedMenu` handles focus management automatically in `OnShow()`/`OnHide()`, you rarely need to call `ChangeGameFocus()` yourself when using this 基底クラス. Simply call `super.OnShow()` and `super.OnHide()`.

---

## 組み込みShowDialog（ネイティブメッセージボックス）
...
## 手動ダイアログパターン（UIScriptedMenuなし）

When you need a dialog that is not part of エンジン menu stack -- 例えば, a popup inside an existing panel -- extend `ScriptedWidgetEventHandler` instead of `UIScriptedMenu`. This gives you full control but requires manual focus and lifecycle management.
...
A reusable confirmation dialog accepts a title, message, and callback. This is the most common dialog pattern DayZ Modにおいて.
...
The callback uses `GameScript.CallFunction()` which invokes a function by name on the target object. これは標準的な方法です DayZ mods implement dialog callbacks since Enforce Script does not support closures or delegates.
...
実際には, most mod developers use `ScriptedWidgetEventHandler`-based popups rather than `UIScriptedWindow`, because the window system requires registering with エンジン's switch-case in `MissionBase` and the events route through the active `UIScriptedMenu`. The manual pattern is simpler and more flexible.
...
**The problem:** Widget tree stays in memory, events keep firing, メモリリークs accumulate.
