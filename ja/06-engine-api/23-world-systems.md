# 第 6.23: World Configuration Systems

[<< 前: Admin & Server Management](22-admin-server.md) | **World Systems**
...
この章では five mission-folder configuration systems:
...
- The PPE effect tints the player's vision (green/yellow tint デフォルトでは)
...
> **注意：** The JSON snippets below include comments for clarity. Real JSON files must not contain comments.
...
| 型 | Breadcrumbs? | EyeAccommodation | 目的 |
|------|-------------|-------------------|---------|
...
| **Inner** | Empty array | `< 1.0` (typically `0.0`) | 定数 darkness deep underground. Eye accommodation is fixed at the configured value |
...
| `Radius` | float | Influence radius in meters. Set to `-1` for エンジン default |
...
> **ヒント:** When using `UseRaycast: 1`, raise the breadcrumb position slightly off the floor (a few centimeters in the Y axis) to avoid the ray being blocked by the ground surface.
...
> **注意：** You only need to include the phenomena you want to override. Omitted phenomena use engine defaults.
...
例： `"0xffbb0a1e"` = fully opaque dark red. The value is not case-sensitive.
...
|---------|-------------|-----|
| JSON comments (`//` or `/* */`) in config files | ファイル fails to parse; features silently disabled | Remove all comments from production JSON files |
