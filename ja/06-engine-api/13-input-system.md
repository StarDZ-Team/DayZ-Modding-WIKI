...
[Home](../../README.md) | [<< 前: Action System](12-action-system.md) | **Input System**
...
1. **inputs.xml** (config layer) --- declares named actions, assigns default keybindings, and organizes them into groups for the player's Controls settings menu. [第 5.2: inputs.xml](../05-config-files/02-inputs-xml.md) for full coverage.

2. **UAInput API** (script layer) --- queries input state at runtime. This is what your scripts call 毎フレーム to detect presses, releases, holds, and analog values.

この章では the script layer: the classes, methods, and patterns you use to read and control inputs from Enforce Script.
...
- **`LocalHold()`** does NOT fire immediately. It waits for エンジン's hold threshold to be met first. Use `LocalPress()` if you need instant response.
...
- `PlayerBase.CommandHandler()` --- runs 毎フレーム for the local player
...
The mission system provides named exclude groups defined in エンジン's `specific.xml`. When activated, they disable categories of inputs:
...
Expansion uses its own custom exclude group registered with エンジン:
...
1. At startup, エンジン reads all `inputs.xml` files from loaded mods
...
The engine method is `Supress()` with a single 's' (not `Suppress`). The global method `SupressNextFrame()` also uses a single 's'. This is a quirk of エンジン API:
...
*この章では the script-side Input System API. For the XML configuration that registers keybindings, see [Chapter 5.2: inputs.xml](../05-config-files/02-inputs-xml.md).*
...
- **Check inputs only in per-frame callbacks like `OnUpdate()`.** `LocalPress()` returns true for exactly one frame. Checking it in event handlers or callbacks that do not run 毎フレーム will miss key presses.
