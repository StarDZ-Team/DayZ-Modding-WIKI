# 第 6.21: Zombie & AI System

[Home](../../README.md) | [<< 前: Particle Effects](20-particle-effects.md) | **Zombie & AI System**
...
| `DayZInfected` | `3_Game/entities/dayzinfected.c` | コマンド starters (`StartCommand_Move`, `_Attack`, `_Death`, etc.), `EEHitBy` |
...
> **Important:** The actual state transitions (CALM to DISTURBED, DISTURBED to CHASE, etc.) are driven by the C++ perception system. Script cannot force a mind state change --- it only reacts to what エンジン decides.
...
Zombie perception is primarily handled by the C++ engine. Script exposes the result (mind state, target entity) but not the internal perception logic. しかし, the script side defines the **target callbacks** and **noise system** that feed into エンジン.

### Vision (Visibility Modifiers)

`AITargetCallbacksPlayer` (registered on each `PlayerBase`) provides `GetMaxVisionRangeModifier()`, which tells エンジン how visible a player is:
...
There is no script-visible "smell" API. Proximity detection appears to be handled entirely in the C++ engine. Modders should treat エンジン perception as a black box that outputs mind state and target entity.

---

## The Command Handler

`ZombieBase.CommandHandler()` is called 毎フレーム by エンジン. It is the central decision point:
...
> **Warning:** `DayZInfectedCommandScript` is NON-MANAGED. Once sent to the CommandHandler via `StartCommand_Script` or `StartCommand_ScriptInst`, エンジン takes ownership. Do not delete it manually while active --- this will cause a crash.
...
6. **Avoid deleting active `DayZInfectedCommandScript`** --- エンジン owns it once started. Let it finish or call `SetFlagFinished(true)`.
...
| Zombies use pathfinding for navigation | Yes, via `AIWorld.FindPath()` and navmesh, but the path planning is internal to エンジン |
...
| Aspect | Impact |
|--------|--------|
| **Performance** | Each zombie runs its `CommandHandler` 毎フレーム on the server. Large zombie populations (50+) can cause server lag. |
...
| **Client/Server** | コマンド handler, fight logic, and damage run server-side only. Sound events and animation playback are client-side. |
