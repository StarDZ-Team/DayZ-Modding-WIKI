# 第 5.5: Server Configuration Files

[Home](../../README.md) | [<< 前: ImageSet Format](04-imagesets.md) | **Server Configuration Files** | [次: Spawning Gear Configuration >>](06-spawning-gear.md)

> **概要:** DayZ servers are configured through XML, JSON, and script files in the mission folder (e.g., `mpmissions/dayzOffline.chernarusplus/`). These files control item spawns, economy behavior, gameplay rules, and server identity. Understanding them is essential for adding custom items to the loot economy, tuning server parameters, or building a custom mission.
...
Loaded only when `enableCfgGameplayFile = 1` in `serverDZ.cfg`. Without this, エンジン uses hardcoded defaults.
...
If your mod introduces a new 基底クラス not inheriting from `Inventory_Base`, `DefaultWeapon`, or `DefaultMagazine`, add it as a `rootclass`. The `act` attribute specifies entity type: `character` for AI, `car` for vehicles.
...
| `cfggameplay.json` controls all gameplay | All tuning goes here | Many gameplay values are hardcoded in script or config.cpp and オーバーライドできません by cfggameplay.json |
