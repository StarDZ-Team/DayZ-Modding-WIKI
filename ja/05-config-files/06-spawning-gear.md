# 第 5.6: Spawning Gear Configuration

[Home](../../README.md) | [<< 前: Server Configuration Files](05-server-configs.md) | **Spawning Gear Configuration**

> **概要:** DayZ has two complementary systems that control how players enter the world: **spawn points** determine *where* a character appears on the map, and **spawn gear** determines *what equipment* they carry. この章では both systems in depth, including file structure, field reference, practical presets, and mod integration.
...
This top-level array defines items that go into the character's **cargo** --- any available inventory space across all attached clothing and containers. Unlike `attachmentSlotItemSets`, these items are not placed into a specific slot; エンジン finds room automatically.
...
Complex children are items spawned **inside** a parent item with full control over their attributes, quickbar assignment, and their own nested children. The primary use case is spawning items with contents --- 例えば, a weapon with attachments, or a cooking pot with food inside.
...
- **`false`** --- Items use エンジン's configuration defaults (typically full health and quantity).
...
When both min and max are specified, エンジン picks a random value in that range. This creates natural variation --- 例えば, health between `0.45` and `0.65` means items spawn in worn to damaged condition.
...
Around every `<pos>` defined in `generator_posbubbles`, エンジン creates a rectangle of `grid_width` x `grid_height` meters, samples it at `grid_density` frequency, and discards points that overlap with objects, water, or exceed slope limits.
...
> **Position format:** The `x` and `z` attributes use DayZ world coordinates. `x` is east-west, `z` is north-south. The `y` (height) coordinate is not specified --- エンジン places the point on the terrain surface. You can find coordinates using the in-game debug monitor or the DayZ Editor mod.
