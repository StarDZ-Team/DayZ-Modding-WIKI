# 第 8.6: Debugging & Testing Your Mod

> **概要:** This tutorial teaches you how to debug and test DayZ mods effectively. You will learn to read script logs, use Print debugging, work with DayZDiag and file patching for rapid iteration, diagnose common error messages, and establish a reliable testing workflow. No IDE debugger exists for retail builds -- your tools are script logs, Print statements, Workbench, DayZDiag, and file patching.
...
| ログ Entry | 意味 |
|-----------|---------|
| `SCRIPT (E): Cannot convert string to int` | 型 mismatch -- passing or assigning the wrong type |
...
| `SCRIPT (E): Method 'GetHelth' not found in class 'EntityAI'` | Method does not exist -- check spelling and 親クラス |
...
| 機能 | Retail DayZ | DayZDiag |
|---------|-------------|----------|
| ファイル patching support | No | Yes |
...
### 使用するタイミング DayZDiag vs Retail

- **During development:** Always use DayZDiag. It gives you file patching, better errors, and debug tools.
- **Before release:** Test with the retail DayZ executable to make sure everything works for players. Some behaviors differ between DayZDiag and retail (例えば, `#ifdef DEVELOPER` code will not run in retail).

---

## File Patching -- Edit Without Rebuilding

File patching is the single biggest time-saver DayZ Moddingにおいて. Without it, every script change requires: edit file, rebuild PBO, copy PBO, restart DayZ. With file patching, you can: edit file, restart the mission. That is it.
...
| Error Message | 原因 | 修正方法 |
|---------------|-------|-----|
| `Null pointer access` | Calling a method on a variable that is NULL | Add a null check before use: `if (obj) { obj.DoThing(); }` |
| `Cannot convert X to Y` | 型 mismatch in assignment or function call | Check the expected type. Use `Class.CastTo()` for safe casting. |
...
| JSON config not loading | ファイル path wrong or JSON syntax error | Check the file path. Validate JSON syntax (no trailing commas, proper quoting). |
...
- **Use a consistent log prefix like `[MyMod]` in every Print call.** Script logs contain output from vanilla code, エンジン, and every loaded mod. Without a prefix, your output is invisible in the noise.
...
| ファイル patching | Edit any file and see changes instantly | `config.cpp` is never file-patched. New `.c` files are not picked up either. Both require a PBO rebuild. Only modifications to existing script and layout files are live-reloaded. |
...
**次:** [Chapter 8.8: Building a HUD Overlay](08-hud-overlay.md)
