# 第 4.7: Workbench Guide

[Home](../../README.md) | [<< 前: PBO Packing](06-pbo-packing.md) | **Workbench Guide** | [次: Building Modeling >>](08-building-modeling.md)
...
| 目的 | 説明 |
|---------|-------------|
| **Script editing** | 構文 highlighting, code completion, and error checking for `.c` files |
...
Workbench uses the same Enfusion script compiler as DayZ itself. When Workbench reports a コンパイルエラー, that error will also occur in-game -- making it a reliable pre-flight check before launching.
...
| Types (`int`, `float`, `string`, `bool`, `vector`, `void`) | 型 color |
...
| `Cannot convert 'string' to 'int'` | 型 mismatch |
| `Type 'MyClass' not found` | ファイル not in project |
...
> **ヒント:** If you only need to see console output (no breakpoints or stepping), you do not need to extract PBOs or load scripts into Workbench. The Script Editor will still connect to DayZDiag and display the Output stream. しかし, breakpoints and code navigation require the matching script files to be loaded in the project.
...
The **Locals** panel shows all variables in scope -- primitives with values, objects with class names (expandable), arrays with lengths, and NULL references clearly marked. The **Watch** panel evaluates custom expressions at each pause. The **Call Stack** shows 関数 chain; click entries to navigate.
...
- **Client-side only** デフォルトでは (server-side code needs a listen server).
...
**ベストプラクティス:** Use the Layout Editor for inspection. Build and edit `.layout` files in a text editor. Test in-game with file patching.
...
| ファイル Type | アクション on Double-Click |
|-----------|----------------------|
...
5. **Set breakpoints strategically.** Avoid `OnUpdate()` breakpoints unless conditional. They fire 毎フレーム and freeze the game constantly.
