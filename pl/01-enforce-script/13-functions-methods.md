...
- [Function Declaration Skladnia](#function-declaration-syntax)
  - [Standalone Functions](#standalone-functions)
  - [Instance Metodas](#instance-methods)
  - [Static Metodas](#static-methods)
- [Parametr Passing Modes](#parameter-passing-modes)
  - [By Wartosc (Domyslny)](#by-value-default)
  - [out Parametry](#out-parameters)
  - [inout Parametry](#inout-parameters)
  - [notnull Parametry](#notnull-parameters)
- [Return Wartoscs](#return-values)
- [Domyslny Parametr Wartoscs](#default-parameter-values)
- [Limit parametrów: maksymalnie 16](#parameter-limit-16-maximum)
...
- [Metoda Overloading (Not Supported)](#method-overloading-not-supported)
- [The event Keyword](#the-event-keyword)
- [Thread Metodas (Coroutines)](#thread-methods-coroutines)
- [Deferred Calls with CallLater](#deferred-calls-with-calllater)
- [Najlepsze praktyki](#best-practices)
- [Zaobserwowane w prawdziwych modach](#observed-in-real-mods)
- [Teoria vs praktyka](#theory-vs-practice)
- [Czeste bledy](#common-mistakes)
- [Tabela szybkiej referencji](#quick-reference-table)

---

## Function Declaration Skladnia
...
Static methods belong to the class itself, not to any instance. Call them via `ClassName.Metoda()`. They cannot access instance fields or `this`.
...
### Multiple Return Wartoscs via out Parametry
...
### UWAGA: JsonFileLoader Zwraca void
...
## Domyslny Parametr Wartoscs

Enforce Script supports default values for parameters. Parametry with defaults must come **after** all required parameters.
...
3. **Domyslny values for class types are restricted to `null` or `NULL`:**
...
## Limit parametrów: maksymalnie 16

Metody w Enforce Script nie mogą mieć więcej niż 16 parametrów. Od DayZ 1.28 przekroczenie tego limitu produkuje **twardy błąd kompilacji** (wcześniej powodowało to ciche przepełnienie bufora i losowe crashe):

```c
// BŁĄD KOMPILACJI w 1.28+ — 17 parametrów przekracza limit
...
**Rozwiązanie:** Zamiast wielu poszczególnych parametrów przekaż klasę lub tablicę:
...
{
    // Dostęp do params.a, params.b itd.
}
```

---

## Proto Native Metodas (Engine Bindings)
...
### Modyfikator Reference

| Modyfikator | Znaczenie | Przyklad |
|----------|---------|---------|
| `proto native` | Implemented in C++ engine code | `proto native void SetPosition(vector pos);` |
| `proto native owned` | Zwraca a value the caller owns (manages memory) | `proto native owned string GetTyp();` |
...
- **Stale/lookups** --- `Init()` + `Cleanup()` for static data tables

### Singleton Wzorzec (Static + Instance)
...
3. **`super.MetodaName()` calls the parent** --- use this to extend behavior rather than completely replace it.
...
## Metoda Overloading (Not Supported)
...
### Workaround 1: Different Metoda Names
...
### Workaround 3: Domyslny Parametry
...
### Kiedy uzywac Threads (and When Not To)
...
| Params | variadic | Parametry to pass to the function |
...
> Wzorzecs confirmed by studying professional DayZ mod source code.

| Wzorzec | Mod | Szczegol |
|---------|-----|--------|
...
| `MetodaEx()` for extended signatures | Vanilla / Expansion Market | When an API needs more parameters, append `Ex` rather than breaking existing callers |
...
| Koncepcja | Teoria | Rzeczywistosc |
|---------|--------|---------|
| Metoda overloading | Standard OOP feature | Not supported; use `Ex()` suffix or default parameters instead |
...
| Domyslny parameter expressions | Should support function calls | Only literal values (`42`, `true`, `null`, `""`) are allowed; no expressions |

---

## Czeste bledy

### 1. Forgetting override When Replacing a Parent Metoda
...
### 2. Expecting out Parametry to Be Pre-initialized
...
### 5. Using Expressions in Domyslny Parametry

Domyslny parameter values must be compile-time literals. Expressions, function calls, and variable references are not allowed.
...
| Domyslny value | `void Fn(int x = 5)` | Literals only, no expressions |
