...
> **Podsumowanie:** This tutorial teaches you how to debug and test DayZ mods effectively. You will learn to read script logs, use Print debugging, work with DayZDiag and file patching for rapid iteration, diagnose common error messages, and establish a reliable testing workflow. No IDE debugger exists for retail builds -- your tools are script logs, Print statements, Workbench, DayZDiag, and file patching.
...
- [Print Debugging (The Reliable Metoda)](#print-debugging-the-reliable-method)
...
- [Common Error Wzorzecs and Solutions](#common-error-patterns-and-solutions)
...
| Log Entry | Znaczenie |
|-----------|---------|
| `SCRIPT (E): Cannot convert string to int` | Typ mismatch -- passing or assigning the wrong type |
...
| `SCRIPT (E): Metoda 'GetHelth' not found in class 'EntityAI'` | Metoda does not exist -- check spelling and parent class |
...
## Print Debugging (The Reliable Metoda)
...
### Kiedy uzywac DayZDiag vs Retail
...
- **Skladnia highlighting** -- Keywords, types, strings, and comments are color-coded
- **Code completion** -- Typ a class name followed by a dot to see available methods
- **Error highlighting** -- Skladnia errors are underlined in red before you run anything
...
## Common Error Wzorzecs and Solutions

Reference tables of the most common errors and how to fix them. Bookmark this section.

### Bledy skryptow

| Error Wiadomosc | Przyczyna | Rozwiazanie |
|---------------|-------|-----|
| `Null pointer access` | Calling a method on a variable that is NULL | Add a null check before use: `if (obj) { obj.DoThing(); }` |
| `Cannot convert X to Y` | Typ mismatch in assignment or function call | Check the expected type. Use `Class.CastTo()` for safe casting. |
...
| `Metoda 'xyz' not found in class 'Abc'` | Calling a method that does not exist on that class | Check the class hierarchy. Look up the correct method name in the API. |
...
| `Typ 'MyClass' not found` | The file defining MyClass is not loaded or is in a higher script layer | Check config.cpp script loading order. Lower layers cannot see higher layers. |
...
### Null Pointers and Safe Casting -- Szczegoled Przyklads
...
Blads every DayZ modder makes at least once. Learn from them.
...
For RPC/client-server issues, the error is often on one side and objawu on the other. Check both `%LocalAppData%\DayZ\` (client) and your server's profile folder.
...
- **Always check both client and server logs for RPC issues.** The error is often on one side and objawu on the other. A server-side null pointer silently drops the response, and the client just sees "no data received."
...
**Nastepny:** [Chapter 8.8: Building a HUD Overlay](08-hud-overlay.md)
