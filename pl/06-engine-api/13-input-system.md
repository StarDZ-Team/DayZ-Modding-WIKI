...
[Strona glowna](../../README.md) | [<< Poprzedni: Akcja System](12-action-system.md) | **Input System**
...
| Class | Source File | Przeznaczenie |
|-------|-----------|---------|
...
| Metoda | Zwraca | Kiedy true | Przypadek uzycia |
|--------|---------|-----------|----------|
...
| `LocalWartosc()` | `float` | Always (returns current value) | Mouse axes, gamepad triggers, analog input |

### Input Class Metodas

| Metoda | Zwraca | Signature | Equivalent UAInput Metoda |
|--------|---------|-----------|--------------------------|
...
| `LocalWartosc()` | `float` | `LocalWartosc(string action, bool check_focus = true)` | `UAInput.LocalWartosc()` |

### Important Timing Uwagi
...
### UAInput vs OnKeyPress: Kiedy uzyc ktorego

| Funkcja | UAInput (GetUApi) | OnKeyPress |
|---------|-------------------|------------|
...
| Modyfikators | `KC_LSHIFT`, `KC_RSHIFT`, `KC_LCONTROL`, `KC_RCONTROL`, `KC_LMENU` (left Alt), `KC_RMENU` (right Alt) |
| Nawigacja | `KC_UP`, `KC_DOWN`, `KC_LEFT`, `KC_RIGHT` |
...
**Wazne:** Always call `GetUApi().UpdateControls()` after modifying input states in bulk.
...
The `bForceSupress` parameter on `RemoveActiveInputExcludes` calls `SupressNastepnyFrame` internally to prevent input bleed when re-enabling.
...
3. Domyslny key bindings from `<preset>` are applied (unless the player has customized them)
...
### Using OnKeyPress for Player-Facing Akcjas
...
### Forgetting UpdateControls After Bulk Zmianas
...
The engine method is `Supress()` with a single 's' (not `Suppress`). The global method `SupressNastepnyFrame()` also uses a single 's'. This is a quirk of the engine API:
...
- **Remember that `Supress()` uses a single "s".** The engine API spells it `Supress()` and `SupressNastepnyFrame()`. Using the correct English spelling `Suppress` will not compile.
