...
[Strona glowna](../../README.md) | [<< Poprzedni: Construction System](17-construction-system.md) | **Animation System**
...
These constants identify the player's current body posture. Defined in `DayZPlayerStale` (scripts/3_game/dayzplayer.c):
...
Bitmask flags used by `IsPlayerInStance()` and `StartCommand_Akcja()` to specify which stances an animation supports:
...
| `GetCommand_Akcja()` | `HumanCommandAkcjaCallback` | Full-body action animations |
...
Modyfikator command IDs (additive, always-on):
...
DayZ's gesture system lets players perform social animations (wave, point, sit, dance, surrender, suicide, etc.). It is built on three layers: `EmoteStale` (IDs), `EmoteBase` (per-emote configuration), and `EmoteManager` (playback orchestration).

### EmoteStale

All emote IDs are defined in `EmoteStale` (scripts/3_game/constants.c):
...
Przyklad -- the greeting emote supports additive playback in erect/crouch and full-body in prone:
...
- **Additive (modifier):** Overlaid on top of locomotion. Player can still move. Uses `AddCommandModyfikator_Akcja()`. Triggered when the player is in a stance matching `m_StanceMaskAdditive`.
- **Full-body:** Takes over the entire animation state. Player cannot move. Uses `StartCommand_Akcja()`. Triggered when the player is in a stance matching `m_StanceMaskFullbody`.
...
### Script-Driven Object Animation Przyklad
...
### HumanCommandAkcjaCallback

All player actions (eating, bandaging, crafting, emotes) use animation callbacks. `HumanCommandAkcjaCallback` (scripts/3_game/human.c) is the base class:
...
Akcjas go through defined states accessible via `GetState()`:
...
| `STATE_LOOP_ACTION` | 6 | Akcja within loop |
| `STATE_NORMAL` | 7 | One-time (non-looping) action |

### Internal Akcja Commands
...
### Starting Akcjas with Animations

Full-body actions use `StartCommand_Akcja()`, additive actions use `AddCommandModyfikator_Akcja()`:
...
Access via `player.GetCommandModyfikator_Additives()`. These are always active and blend on top of whatever command is running.

---

## DayZPlayerStale Szybka referencja

### Common Akcja Animation IDs
...
4. **Keep animation phase values between 0.0 and 1.0.** The engine interpolates between `minWartosc` and `maxWartosc` defined in `model.cfg`. The phase is always normalized.
...
6. **For emotes, go through EmoteManager.** Direct `StartCommand_Akcja()` with gesture IDs works in debug tools but bypasses the sync, interrupt, and state management logic that `EmoteManager` provides.
...
- **COT (Community Online Tools)** uses `StartCommand_Akcja()` directly in its camera/cinematic tools to force gesture animations on players for screenshots.
...
**Practice:** Most modders never need it. The action system (`StartCommand_Akcja` / `AddCommandModyfikator_Akcja`) with pre-existing animation IDs covers 95% of use cases. `HumanCommandScript` is reserved for edge cases like custom vehicle types or entirely new movement modes.

**Teoria:** You can register custom animation events and receive them in callbacks.
**Practice:** Custom animation events require custom animation files (`.rtm` / animation graph changes), which need Workbench and the DayZ animation toolchain. Most modders reuse existing animation IDs and rely on state change callbacks (`OnStateZmiana`, `OnFinish`) rather than custom events.
...
2. **Forgetting that action commands replace movement.** `StartCommand_Akcja()` is full-body -- the player stops moving. If you want an overlay animation, use `AddCommandModyfikator_Akcja()` instead.
...
4. **Mixing up stance index and stance mask.** `ForceStance()` takes a `STANCEIDX_*` value (integer 0-5). `StartCommand_Akcja()` and `IsPlayerInStance()` take `STANCEMASK_*` values (bitmask flags). Passing one where the other is expected produces silent, wrong behavior.
...
- The animation command system is **engine-level** and has remained stable across DayZ versions. `HumanCommandMove`, `HumanCommandAkcjaCallback`, and `DayZPlayerStale` have not changed their API in years.
...
| File | Content |
|------|---------|
| `scripts/3_game/human.c` | HumanInputController, HumanCommandAkcjaCallback, HumanCommandMove, HumanCommandFall, HumanCommandClimb, HumanCommandSwim, HumanCommandVehicle, HumanCommandScript, HumanMovementState, HumanAnimInterface, Human class |
| `scripts/3_game/dayzplayer.c` | DayZPlayerStale (stances, commands, action/gesture IDs) |
| `scripts/3_game/constants.c` | EmoteStale (emote IDs, suicide event IDs) |
