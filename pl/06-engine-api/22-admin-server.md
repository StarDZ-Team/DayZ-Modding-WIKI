...
[Strona glowna](../../README.md) | [<< Poprzedni: Zombie & AI System](21-zombie-ai-system.md) | **Admin & Server Management** | [Nastepny: World Systems >>](23-world-systems.md)
...
| `OnContinouousAkcja(action_data)` | Build/dismantle/destroy actions |
...
The `colorClass` parameter maps to config entries. Common values are `"colorAkcja"`, `"colorFriendly"`, `"colorImportant"`.
...
**Przyklad usage** (pattern from VPP TimeManager):
...
| Event Typ | Metoda Called | When |
|------------|-------------|------|
| `ClientPrepareEventTypID` | `OnClientPrepareEvent()` | Player begins connecting |
| `ClientNewEventTypID` | `OnClientNewEvent()` | New character created |
| `ClientReadyEventTypID` | `OnClientReadyEvent()` | Existing character loaded |
| `ClientReconnectEventTypID` | `OnClientReconnectEvent()` | Reconnecting to existing character |
| `ClientDisconnectedEventTypID` | `OnClientDisconnectedEvent()` | Player disconnecting |
...
The primary server-side audit log. Controlled by `serverDZ.cfg` settings (`adminLogPlayerHitsOnly`, `adminLogPlacement`, `adminLogBuildAkcjas`, `adminLogPlayerList`). Written to `$profile:` as `.ADM` files.
...
> **Uwaga:** The DayZ engine does not provide a native HTTP client. Webhook functionality relies on the CF (Community Framework) `RestApi` system, which provides an HTTP callback mechanism. This is an external dependency, not a vanilla engine feature.
...
### 3. Setting Weather Without MissionWeather Flaga
