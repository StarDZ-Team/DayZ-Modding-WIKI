...
[Strona glowna](../../README.md) | [<< Poprzedni: PBO Packing](06-pbo-packing.md) | **Workbench Guide** | [Nastepny: Building Modeling >>](08-building-modeling.md)
...
- [Pliki projektu (.gproj)](#project-files-gproj)
- [Interfejs Workbench](#the-workbench-interface)
- [Edycja skryptow](#script-editing)
- [Debugowanie skryptow](#debugging-scripts)
- [Script Console -- Live Testing](#script-console----live-testing)
- [UI / Layout Preview](#ui--layout-preview)
- [Przegladarka zasobow](#resource-browser)
- [Profilowanie wydajnosci](#performance-profiling)
- [Integration with File Patching](#integration-with-file-patching)
- [Typowe problemy Workbench](#common-workbench-issues)
- [Wskazowki i najlepsze praktyki](#tips-and-best-practices)
...
| Przeznaczenie | Opis |
|---------|-------------|
| **Script editing** | Skladnia highlighting, code completion, and error checking for `.c` files |
...
| Module | Layer | EntryPoint | Przeznaczenie |
|--------|-------|------------|---------|
...
- **Przegladarka zasobow** (left) -- File tree of the P: drive. Double-click `.c` files to edit, `.layout` files to preview, `.p3d` to view models, `.paa` to view textures.
...
1. **Przegladarka zasobow:** Double-click a `.c` file. This automatically opens the Script Editor module and loads the file.
2. **Script Editor Przegladarka zasobow:** The Script Editor has its own built-in Przegladarka zasobow panel, separate from the main Workbench Przegladarka zasobow. You can use either to navigate and open script files.
3. **File > Open:** Standard file dialog.
4. **Error output:** Double-click a compiler error to jump to the file and line.

### Skladnia Highlighting

| Element | Podswietlone |
|---------|-------------|
...
2. Open the **Script Editor** module (from the menu bar or by double-clicking any `.c` file in the Przegladarka zasobow -- this automatically opens the Script Editor and loads the file).
...
> **Wskazowka:** If you only need to see console output (no breakpoints or stepping), you do not need to extract PBOs or load scripts into Workbench. The Script Editor will still connect to DayZDiag and display the Output stream. However, breakpoints and code navigation require the matching script files to be loaded in the project.
...
## Przegladarka zasobow

The Przegladarka zasobow navigates the P: drive with game-aware file previews.

### Capabilities

| Typ pliku | Akcja on Double-Click |
|-----------|----------------------|
...
### Typowe waskie gardla

| Problem | Objaw profilera | Rozwiazanie |
|---------|-----------------|-----|
...
## Typowe problemy Workbench

### Workbench Crashes on Startup

**Przyczyna:** P: drive not mounted or `.gproj` references nonexistent paths.
**Rozwiazanie:** Mount P: first. Check **Workbench > Options** source directory. Verify `.gproj` FileSystem paths exist.

### No Code Completion

**Przyczyna:** Project misconfigured -- Workbench cannot compile scripts.
**Rozwiazanie:** Verify `.gproj` ScriptModules include vanilla paths (`scripts/1_Core`, etc.). Check Output for compiler errors. Ensure vanilla scripts are on P:.

### Scripts Do Not Compile

**Rozwiazanie:** Check Output panel for exact errors. Verify all dependency mod paths are in ScriptModules. Ensure no cross-layer references (3_Game cannot use 4_World types).
...
### Cannot Find Files in Przegladarka zasobow

**Rozwiazanie:** Check `.gproj` FileSystem includes the directory where your files live. Restart Workbench after modifying `.gproj`.

### "Plugin Not Found" Errors

**Rozwiazanie:** Verify DayZ Tools integrity via Steam (right-click > Properties > Installed Files > Verify). Reinstall if needed.

### Connection to DayZDiag Fails

**Rozwiazanie:** Both processes must be on the same machine. Check firewalls. Ensure Script Editor module is open before launching DayZDiag. Try restarting both.
...
7. **Check compiler output before launching.** If Workbench reports errors, the game will fail too. Rozwiazanie errors in Workbench first -- faster than waiting for game boot.
...
## Szybka referencja: Keyboard Skrots
...
## Szybka referencja: Launch Parametry
