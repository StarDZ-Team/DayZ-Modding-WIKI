...
> **Podsumowanie:** DayZ is a client-server game. Every line of code you write runs in a specific context -- server, client, or both. Understanding this split is essential for writing secure, functional mods. This chapter explains where code runs, how to detect which side you are on, how to structure multi-package mods, and the patterns that keep server and client code properly separated.
...
| Metoda | Dedicated Server | Client (Remote) | Listen Server |
|--------|:---:|:---:|:---:|
...
**Przyklad:** The StarDZ AI client mod uses `type = "mod"` because both server and client need the AI entity class definitions, RPC constants, and sync data structures:
...
**Przyklad:** The StarDZ AI Server mod uses `type = "servermod"` because clients should never see the AI brain, perception, combat, or spawning logic:
...
| Layer | Dedicated Server | Client | Listen Server | Uwagi |
|-------|:---:|:---:|:---:|-------|
...
### Kiedy uzywac Preprocessor Guards vs Runtime Checks

| Podejscie | Kiedy uzywac | Przyklad |
|----------|-------------|---------|
...
### Real Przyklad: Client Mission Hook in a Shared Mod
...
## Common Server-Client Wzorzecs

### Wzorzec 1: Server-Side Validation with Client Feedback
...
### Wzorzec 2: Config Sync (Server to Client)
...
**Rozwiazanie:** If you need exclusive branches, use `else if` or check `IsDedicatedServer()`:
...
**Rozwiazanie:** Use server/client specific subclasses or guard with context checks:
...
- Typs that exist only in `MissionGameplay` are available on listen server
...
## Real-World Split Przyklads

### Przyklad 1: StarDZ AI (Client + Server)
...
### Przyklad 3: StarDZ Missions (Client + Server)
...
### Blad 1: Running Server Logic on Client
...
### Blad 2: UI Code in Server-Only Mod
...
**Rozwiazanie:** All UI code belongs in the client package (`type = "mod"`), wrapped in `#ifndef SERVER`.

### Blad 3: GetGame().GetPlayer() on Server
...
**Rozwiazanie:** On the server, players are passed to you through events, RPCs, or iteration:
...
### Blad 4: Forgetting Listen Server Compatibility
...
### Blad 5: Not Using #ifdef for Optional Mod Detection
...
### Blad 6: Putting Shared Typs Only in the Server Package
...
**Rozwiazanie:** Shared data structures (RPC data, entity definitions, config classes) go in the client package (`type = "mod"`) so both sides have them:
...
### Blad 7: Hardcoded Server File Paths on Client
...
**Rozwiazanie:** The server loads configs and sends relevant data to clients via RPC. Clients never read server config files directly.
...
**Poprzedni:** [Chapter 2.5: File Organization Najlepsze praktyki](05-file-organization.md)
**Nastepny:** [Part 3: GUI & Layout System](../03-gui-system/01-widget-types.md)
