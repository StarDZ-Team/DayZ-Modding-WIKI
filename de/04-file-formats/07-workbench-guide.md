...
Workbench is Bohemia's IDE for Enfusion engine development. Es ist der/die/das only tool in the DayZ Tools suite that can compile, analyze, and debug Enforce Script. It serves six purposes:
...
- **Not erforderlich for building PBOs.** AddonBuilder and build scripts handle PBO packing independently.
...
1. Navigate to your DayZ installation folder (typischerweise `Steam\steamapps\common\DayZ`).
...
**ScriptModules** -- The most wichtig section. Maps each engine layer to script directories:
...
1. **Resource Browser:** Double-click a `.c` file. This automatisch opens the Script Editor module and loads the file.
...
- **Scripts must match** -- if you edit after game loads, line numbers wird nicht align.

### Setting Up a Debug Session

1. Open Workbench and load your project.
2. Open the **Script Editor** module (from the menu bar or by double-clicking any `.c` file in the Resource Browser -- this automatisch opens the Script Editor and loads the file).
...
> **Tip:** If you only need to see console output (no breakpoints or stepping), you mache nicht need to extract PBOs or load scripts into Workbench. The Script Editor will still connect to DayZDiag and display the Output stream. Jedoch, breakpoints and code navigation require the matching script files to be loaded in the project.
...
Toggle with `F9`. You can also left-click directly in the margin area (where the red dots appear) to add or remove breakpoints. Right-clicking in the margin adds a blue **Bookmark** stattdessen -- bookmarks have no effect on execution but mark places you want to revisit. Right-click a breakpoint to set a **condition** (e.g., `i == 10` or `player.GetIdentity().GetName() == "TestPlayer"`).
...
The **Locals** panel shows all variables in scope -- primitives with values, objects with class names (expandable), arrays with lengths, and NULL references clearly marked. The **Watch** panel evaluates custom expressions at each pause. The **Call Stack** shows die Funktion chain; click entries to navigate.
...
- **No runtime behavior** -- ScriptClass handlers and dynamic content mache nicht execute.
...
**Fix:** Both processes muss on the same machine. Check firewalls. Ensure Script Editor module is open before launching DayZDiag. Try restarting both.
...
5. **Set breakpoints strategically.** Avoid `OnUpdate()` breakpoints unless conditional. They fire every frame and freeze das Spiel constantly.
...
## Kurzreferenz: Keyboard Shortcuts
...
## Kurzreferenz: Launch Parameters
