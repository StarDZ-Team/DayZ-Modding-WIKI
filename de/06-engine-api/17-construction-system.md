...
[Home](../../README.md) | [<< Previous: Crafting System](16-crafting-system.md) | **Construction System**
...
| `m_RequiredParts` | `array<string>` | Parts that muss built first |
...
1. Part ist nicht already built (`!IsPartConstructed`)
...
Each material entry in config specifies `type`, `slot_name`, `quantity`, and `lockable`. `HasMaterials()` checks that each slot has an attachment with at least the erforderlich quantity. For repairs, quantity is reduced to 15% (`REPAIR_MATERIAL_PERCENTAGE = 0.15`), minimum 1.
...
- **Lockable** (e.g., barbed wire): slot is locked, item stays attached but kann nicht removed
- **Quantity-based**: quantity subtracted from attachment stack
- **Delete** (quantity = -1): entire attachment is deleted

`OnPartBuiltServer()` then: registers the part bit, syncs to clients, updates visuals/physics/navmesh, and if the part `IsBase()`, marks die Basis state and spawns a construction kit.

### Synchronization

Part states sync via three `int` bitmask variables (`m_SyncParts01/02/03`). Der Server manages these with:
...
After changing bits, der Server calls `SynchronizeBaseState()` which triggers `SetSynchDirty()`. On the client, `OnVariablesSynchronized()` fires `SetPartsFromSyncData()`, iterating all parts and updating built states, visuals, and physics.
...
On load, `AfterStoreLoad()` calls `SetPartsAfterStoreLoad()` which reconstructs all part states from the bitmask, restores die Basis state flag, and synchronizes.
...
Dismantling die Basis part triggers `DestroyConstruction()` (deletes the entire entity) after a 200ms delay.
...
Exception: gate parts sind nicht cascade-destroyed if either `wall_base_down` or `wall_base_up` is still built.
...
Full-body continuous action. Duration: `UATimeSpent.BASEBUILDING_CONSTRUCT_MEDIUM`. Erfordert non-ruined tool in hand. Uses the **variant system** --- `ConstructionActionData.OnUpdateActions()` populates a list of buildable parts for the targeted component, and `m_VariantID` selects which part to build.
...
- Part kann nicht on a locked gate (combination lock or flag attached)
- Gate parts kann nicht dismantled while the gate is opened
- Camera direction and player position checks prevent dismantling from the wrong side
- Player kann nicht prone
...
The `OnUpdateActions()` callback is registered with `ActionVariantManager` and fires when der Spieler looks at a construction object, populating the build part list for the action variant system.
...
Multi-level tower. Kit: `WatchtowerKit`. Up to 3 floors with walls and roofs per level. Constants: `MAX_WATCHTOWER_FLOORS = 3`, `MAX_WATCHTOWER_WALLS = 3`. Has a `MAX_FLOOR_VERTICAL_DISTANCE = 0.5` check preventing attachment of items from too far below das Ziel floor.
...
Dies ist der/die/das raiding mechanism: explosives and weapons deal damage to damage zones until they reach ruined state. Barbed wire attachments have special handling --- when their zone is ruined, the wire's mounted state is cleared.
...
5. **Match damage zone names to part names** exactly (case-insensitive) or health/damage wird nicht work.
...
7. **Stay under 93 parts** per object. Complex structures sollte split into multiple placeable objects.
...
Mods should use `modded` keyword on `BaseBuildingBase` / `Construction` rather than full replacement. Be besonders careful with the sync flow --- modifying bitmask variables or `RegisterPartForSync` without understanding the full chain causes server-client desynchronization.
