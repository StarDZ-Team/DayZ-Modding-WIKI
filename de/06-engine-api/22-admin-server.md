...
Die Engine provides two equivalent ways to retrieve all connected player entities.
...
DayZ macht nicht expose a direct `KickPlayer()` script function. Stattdessen, die Engine provides `DisconnectPlayer()` which terminates the connection.
...
Admin mods implement kick functionality using this, combined with a notification RPC so der Client sees a reason message before disconnection:
...
The `EClientKicked` enum (from `3_Game/global/errormodulehandler/clientkickedmodule.c`) defines all possible kick reasons die Engine recognizes:
...
On subsequent connection attempts, VPP checks the ban list during `ClientPrepareEvent` and refuses entry by scheduling a kick before der Spieler fully loads.
...
Die Engine provides a native method to write to the server's admin log file (`*.ADM` file in der Server profile).
...
Every admin command muss validated server-side. The pattern is:
...
Never trust client-side permission checks alone. Always re-validate auf dem Server.
...
Der Server entry point is `init.c`, located in the mission folder (e.g., `mpmissions/dayzOffline.chernarusplus/`). It creates and assigns the mission:
...
The `$profile:` path prefix resolves to the server's profile directory (set via `-profiles=` launch parameter). Dies ist der/die/das primary location for server-side data:
...
Scripts can read values from `serverDZ.cfg` zur Laufzeit:
...
The primary server-side audit log. Gesteuert durch `serverDZ.cfg` settings (`adminLogPlayerHitsOnly`, `adminLogPlacement`, `adminLogBuildActions`, `adminLogPlayerList`). Written to `$profile:` as `.ADM` files.
...
The vanilla `PluginAdminLog` class provides structured logging. It automatisch formats player prefixes with name, Steam ID, and position:
...
> **Note:** The DayZ engine macht nicht provide a native HTTP client. Webhook functionality relies on the CF (Community Framework) `RestApi` system, which provides an HTTP callback mechanism. This is an external dependency, not a vanilla engine feature.
...
The Hive is DayZ's persistence layer --- a native C++ system that stores character data, world objects (tents, barrels, vehicles), and server state to disk. It ist nicht directly accessible from script beyond a small set of `proto native` methods.
...
These are called automatisch by `MissionServer` during connection and disconnection events:
...
World objects (tents, barrels, buried stashes, vehicles) persist through the Central Economy (CE) system, not through direct script calls. The CE reads and writes to the `storage_1/` folder in der Server profile. Scripts can force a save through the Hive but cannot query the persistence database directly.
...
The Hive mode is set in `init.c` before the mission starts. If no Hive is created, `GetHive()` returns null and der Server runs without any persistence.
...
- Aktualisiert run on a timer to avoid excessive network traffic
...
BattlEye is DayZ's anti-cheat system. It runs as a separate process alongside der Server and monitors:
...
2. **Log and kick** der Spieler (restriction level 5)
...
3. Validating permissions server-side (BattlEye only monitors, it macht nicht enforce admin permissions)
...
RCON (Remote Console) is BattlEye's admin interface, separate from script. RCON commands like `#kick`, `#ban`, `#shutdown` are handled by BattlEye directly and sind nicht accessible from Enforce Script.
...
- **Use `$profile:` for all server-side file storage.** Never use hardcoded absolute paths. The `$profile:` prefix adapts to whatever profile directory der Server operator has configured.
...
During `ClientPrepareEvent`, der Spieler entity may not exist yet. During `ClientDisconnectedEvent`, the identity may already be null.
...
`OpenFile()` will fail silently if der Elternteil directory macht nicht exist.
...
- **Performance Impact:** ESP systems that send frequent player position updates can generate significant network traffic. Both COT and VPP use update timers (typischerweise 1-5 second intervals) rather than per-frame updates. The vanilla `PluginAdminLog.PlayerList()` runs every 300 seconds (5 minutes) to minimize overhead.
