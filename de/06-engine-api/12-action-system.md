# Chapter 6.12: Aktion System

[Home](../../README.md) | [<< Previous: Central Economy](10-central-economy.md) | **Action System**
...
Dieses Kapitel behandelt the full API, class hierarchy, lifecycle, and practical patterns for creating custom actions.
...
### Key Differences Between Aktion Types
...
The `ActionTarget` class represents what der Spieler is aiming at:
...
The `ActionTargets` class runs each frame auf dem Client, gathering potential targets:

1. **Raycast** from camera position along camera direction (`c_RayDistance`)
2. **Vicinity scan** for nearby objects around the player
3. For each candidate, die Engine calls `GetActions()` on das Objekt to find registered actions
...
Steuert whether der Gegenstand in the player's hand qualifies for this action.
...
| `CCIDummy` | Passes if item ist nicht null (item must exist) |
| `CCINonRuined` | Passes if item exists AND ist nicht ruined |
| `CCINotPresent` | Passes if item is null (hands muss empty) |
...
Steuert whether das Ziel object (what der Spieler is looking at) qualifies.
...
Actions are registered on entities through the `SetActions()` / `AddAction()` / `RemoveAction()` pattern. Die Engine calls `GetActions()` on an entity to retrieve its action list; the first time this happens, `InitializeActions()` builds the map via `SetActions()`.
...
Player-level actions (drinking from ponds, opening doors, etc.) are registered in `PlayerBase.SetActions()`. Es gibt zwei signatures:
...
4. At runtime, die Engine queries all actions for the matching input type

Das bedeutet actions are shared per **type** (class), not per instance. All items of the same class share the same action list.

---

## Creating a Custom Aktion --- Step by Step

### Beispiel 1: Simple Single-Use Action

A custom action that instantly heals der Spieler when they use a special item:
...
### Beispiel 2: Continuous Aktion with Progress Bar
...
### Beispiel 3: Interact Aktion (World Object Toggle)
...
### Beispiel 4: Aktion with Specific Item Requirement

An action that requires der Spieler to hold a specific tool type while targeting a specific object:
...
## Aktion Components (Progress Control)
...
Repeating cycles --- `OnFinishProgressServer()` is called each time a cycle completes, and the action continues until der Spieler releases the key:
...
- `GetComponentIndex()` retrieves which door der Spieler is looking at
...
### Aktion Condition Masks

Actions kann restricted to specific player states using `ActionConditionMask`:
...
Actions kann **additive** (player can walk) or **full body** (player is locked in place):
...
When `HasProneException()` returns true, die Engine uses `m_CommandUIDProne` stattdessen of `m_CommandUID` if der Spieler is in prone stance.

### Aktion Interruption

Actions kann interrupted server-side through the callback:
...
Actions kann configured to run from the inventory screen or quickbar:
...
This **removes all vanilla actions** from the item. Der Spieler will no longer be able to eat, drop, or andernfalls interact with apples through standard actions.
...
Health changes and inventory operations must happen auf dem Server. `OnExecuteClient` is only for visual feedback (sounds, particle effects, UI updates).
...
**Problem:** Aktion macht nicht show up in the HUD.

Common causes:
- `CCIDummy` requires an item in hand, but the action should work with empty hands --- use `CCINone` stattdessen
- `CCTDummy` requires a target object, but the action is a self-action --- use `CCTSelf` or `CCTNone`
- `CCTObject` distance too small for das Ziel type --- increase distance parameter
...
- `OnStart` / `OnStartServer`: Aufgerufen wenn the action **begins** (animation starts). Use for setup, reserving items.
- `OnExecute` / `OnExecuteServer`: Aufgerufen wenn the animation **event fires** (the "do" moment). Use for the actual effect.
...
### 6. Continuous Aktion Not Repeating

If your continuous action completes once and stops stattdessen of repeating, you are using `CAContinuousTime` (single completion). Switch to `CAContinuousRepeat` for repeating cycles:
...
Without this, die Engine expects a target object and may not show the action, or will try to sync a non-existent target to the server.
...
The DayZ Aktion System follows a consistent pattern:
...
The system is designed to be modular: condition components handle "can this happen?", action components handle "how long does it take?", and your overrides handle "what does it do?". Keep server logic auf dem Server, visual feedback auf dem Client, and always null-check your targets.
...
- **Null-check every object in `ActionCondition()`.** Die Methode is called frequently with potentially null targets. Accessing `.GetObject()` without a guard causes crashes that are hard to diagnose.
- **Prefer `CAContinuousRepeat` over `CAContinuousTime` for repair-style actions.** Repeat fires `OnFinishProgressServer` each cycle and continues until der Spieler releases the key, which feels more natural for ongoing tasks.

---

## Compatibility & Impact

- **Multi-Mod:** Actions are registered per class type via `SetActions()`. Two mods adding different actions to the same item class both work -- actions accumulate. Jedoch, if both mods override `SetActions()` without calling `super`, only the last-loaded mod's actions survive.
...
- **Server/Client:** The action pipeline is split: condition checks and UI display run auf dem Client, execution callbacks run auf dem Server. Die Engine handles synchronization via internal RPCs. Never rely on client-side state for authoritative game logic.
