...
[Home](../../README.md) | [<< Previous: Sound System](15-sound-system.md) | **Crafting System**
...
Es gibt zwei fundamental approaches to crafting in DayZ:

1. **Recipe-based crafting** --- Items are combined through `PluginRecipesManager`, which maintains a registry of `RecipeBase` subclasses. Each recipe declares its ingredients, results, conditions, and modifications. Die Engine automatisch discovers valid recipes when two items are combined and presents them to der Spieler via the `ActionWorldCraft` action.

2. **Action-based crafting** --- Custom `ActionContinuousBase` subclasses handle crafting directly without the recipe system. This approach is used for specialized transformations like basebuilding construction, cooking, and gardening where the recipe abstraction macht nicht fit.
...
| `PluginRecipesManagerBase` | `4_World/classes/recipes/recipes/pluginrecipesmanagerbase.c` | Registriert all vanilla recipes via `RegisterRecipies()`. |
...
> **Key insight:** Every recipe has exactly two ingredient "slots" (index 0 and index 1). Each slot can accept multiple item types (e.g., slot 0 accepts `Rag` OR `BandageDressing` OR `DuctTape`), but der Spieler always combines exactly two items.

---

## RecipeBase --- The Recipe Definition

Every crafting recipe is a class that extends `RecipeBase`. Der Konstruktor calls `Init()` automatisch, where you configure everything about the recipe.
...
| Field | Default | Description |
|-------|---------|-------------|
| `m_IngredientAddHealth[i]` | `0` | Health delta applied. `0` = mache nichthing. |
| `m_IngredientSetHealth[i]` | `0` | Set health to this value. `-1` = mache nichthing. |
| `m_IngredientAddQuantity[i]` | `0` | Quantity delta applied. `0` = mache nichthing. Negative values consume. |
...
| `m_ResultSetQuantity[i]` | `0` | Set result quantity to this value. `-1` = mache nichthing. |
| `m_ResultSetHealth[i]` | `0` | Set result health. `-1` = mache nichthing. |
| `m_ResultInheritsHealth[i]` | `0` | `-1` = mache nichthing. `>= 0` = inherit from ingredient N. `-2` = average of all ingredients. |
| `m_ResultInheritsColor[i]` | `0` | `-1` = mache nichthing. `>= 0` = append ingredient N's `color` config value to classname. |
| `m_ResultToInventory[i]` | `0` | `-2` = spawn on ground. `-1` = player inventory. `>= 0` = swap position with ingredient N. |
| `m_ResultReplacesIngredient[i]` | `0` | `-1` = mache nichthing. `>= 0` = transfer properties/attachments from ingredient N. |
...
Unlike actions (which muss registered on items via `SetActions()`), recipes are automatisch discovered. The `PluginRecipesManager` builds a cache beim Start by walking all registered recipes and matching them against all config-defined items. When any two items are combined, the cache is queried for matching recipes.
...
Aufgerufen aus the `RecipeBase` constructor. Set all field values here. Do not call `super.Init()` --- it is an empty declaration in die Basis class.
...
Called during `CheckRecipe()` after the built-in condition checks (`CheckConditions()`) pass. Use this for custom validation logic that kann nicht expressed through the field-based conditions. The `ingredients[]` array is **sorted** --- index 0 maps to your ingredient slot 0 definition, index 1 to slot 1.

Die Basis implementation checks whether any ingredient has attachments and returns `false` if so:
...
Aufgerufen nach `SpawnItems()` and `ApplyModifications*()` have run. This is where you implement custom post-craft logic --- transferring properties, modifying state, triggering effects. The built-in field system handles most modifications automatisch; `Do()` is for anything beyond that.
...
Override to return `true` if the recipe should continue repeating after completion (like repair recipes where you keep repairing until der Gegenstand is fixed or the tool runs out):
...
When repeatable, `CAContinuousCraft` returns `UA_PROCESSING` after each cycle stattdessen of `UA_FINISHED`, causing the progress bar to reset and run again.

### OnSelected(ItemBase item1, ItemBase item2, PlayerBase player)

Aufgerufen wenn der Spieler selects this recipe from the recipe list. Rarely overridden --- kann used for preview effects or UI feedback.
...
- `uid` --- Override the crafting animation when this specific ingredient is used. Zum Beispiel `CleanWeapon` uses `CMD_ACTIONFB_CLEANING_WEAPON` when the ingredient is a `DefaultWeapon`.
- `showItem` --- If `true`, der Gegenstand remains visible in the player's hands during the animation. If `false` (default), der Gegenstand is hidden.
...
When you insert a parent class name like `"Inventory_Base"` or `"DefaultWeapon"`, the recipe matches **all subclasses**. Die Engine uses `g_Game.IsKindOf()` for matching, which walks the config hierarchy:
...
Sets die Basis crafting animation for the entire recipe (different from per-ingredient animation overrides):
...
Dies ist der/die/das action that drives all recipe-based crafting. Players mache nicht interact with it directly --- the `CraftingManager` creates it automatisch when valid recipes exist between two items.
...
3. Der Spieler sees recipe names in the action prompt and can cycle through them.
...
- Color inheritance: if `m_ResultInheritsColor[i] >= 0`, das Ergebnis classname is composed as `ResultName + ingredient.ConfigGetString("color")`
...
- Andernfalls apply `m_IngredientAddHealth`, `m_IngredientSetHealth`, and `m_IngredientAddQuantity`
- Quantity reduction that brings an item to zero automatisch destroys it
- Magazine items use `ServerSetAmmoCount()` stattdessen of `AddQuantity()`
...
The recipe cache is a performance-critical system that avoids checking every recipe against every item combination zur Laufzeit.
...
Das bedeutet if you register `"Inventory_Base"` as an ingredient, every item that inherits from `Inventory_Base` gets that recipe in its cache automatisch.

### Cache Lookup

When two items are combined:

1. `GetRecipeIntersection()` finds der Gegenstand with fewer cached recipes
...
Many recipes use one ingredient as a "tool" that ist nicht consumed. Set the tool ingredient to not be destroyed and optionally lose health:
...
If the quantity reduction brings der Gegenstand to zero or below, die Engine destroys it automatisch (via `AddQuantity()` return value).

### Item Transformation (No Spawned Result)

Some recipes transform an ingredient directly stattdessen of spawning a new item. The saw-off recipes use `MiscGameplayFunctions.TurnItemIntoItemEx()` in `Do()` without calling `AddResult()`:
...
The `m_ResultInheritsColor` system creates variant items based on an ingredient's config `color` value. Das Ergebnis classname becomes `AddResult_name + color_string`:
...
5. **Keep `CanDo()` lightweight.** It runs frequently during recipe validation (every frame der Spieler looks at a craftable combination). Avoid heavy operations like config lookups or iteration.
...
- **Two-ingredient limit is absolute.** The system is hard-coded for exactly two ingredient slots. Multi-step crafting (3+ materials) muss split into sequential recipes.
...
- **`m_AnywhereInInventory` is rarely used.** Most recipes require at least one item in hands. This field exists but ist nicht widely exercised.
...
Each field uses different "mache nichthing" values:
...
Mixing these up (e.g., using `0` stattdessen of `-1` for `m_ResultSetHealth`) will set das Ergebnis health to 0, ruining it immediately.

### 3. Incorrect Ingredient Sorting Assumptions

The `ingredients[]` array in `CanDo()` and `Do()` is sorted to match your slot definitions, not the order der Spieler combined items. If you defined `InsertIngredient(0, "Knife")` and `InsertIngredient(1, "Stick")`, then `ingredients[0]` is always the knife and `ingredients[1]` is always the stick, regardless of which item der Spieler held.

### 4. Consuming Items Without Quantity Checks

If you set `m_IngredientAddQuantity[0] = -6` but `m_MinQuantityIngredient[0] = 1`, players can craft with only 1 unit and the quantity goes negative (or der Gegenstand is destroyed). Always ensure min quantity is at least the consumption amount:
...
If your recipe macht nicht appear in game:
...
- Verify damage level conditions sind nicht excluding valid items (many vanilla recipes set `m_MaxDamageIngredient = 3`, blocking ruined items)
...
- Two mods registering recipes for the same ingredient combination will both work --- der Spieler sees all valid recipes and can cycle through them.
- `UnregisterRecipe()` uses class name strings, which is fragile. If a mod renames or removes die Klasse you are trying to unregister, it silently fails.

### Performance

- The recipe cache is built once beim Start. Adding many recipes increases startup time but has no runtime cost.
- `GetValidRecipes()` runs every frame auf dem Client when der Spieler holds an item and looks at another item. The cache makes this fast (O(1) lookup per item pair, then O(n) for matched recipes where n is small).
...
- Client validates recipes locally for UI display but der Server re-validates before executing.
- The `RecipeSanityCheck()` auf dem Server verifies items are within acceptable distance (5 meters) and not owned by another live player.
...
The DayZ crafting system is built on a recipe registry pattern where `RecipeBase` subclasses declare ingredients, conditions, and results through a set of indexed field arrays. `PluginRecipesManager` caches the ingredient-to-recipe mapping beim Start, and `ActionWorldCraft` provides the player-facing crafting action with progress bar and animation support. Custom recipes require only two files: a `RecipeBase` subclass with `Init()`, `CanDo()`, and optionally `Do()` overrides, plus a modded `PluginRecipesManagerBase` to register it. The system is constrained to exactly two ingredients per recipe, but each slot can accept multiple item types via inheritance matching, and recipes can produce up to 10 output items with configurable health, quantity, and property inheritance.
