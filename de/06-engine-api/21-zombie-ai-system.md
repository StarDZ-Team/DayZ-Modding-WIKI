...
[Home](../../README.md) | [<< Previous: Particle Effects](20-particle-effects.md) | **Zombie & AI System**
...
Dieses Kapitel behandelt the full class hierarchy, the mind state machine, movement and attack commands, the perception/targeting system, spawning patterns, and modding hooks. All method signatures and constants are taken directly from the vanilla script source. Where behavior is driven by the C++ engine with no script-visible API, that ist nichted explicitly.
...
Definiert in `3_Game/entities/dayzinfected.c`:
...
| **ALERTED** | `MINDSTATE_ALERTED` | Strong stimulus confirmed. Active searching. Verwendet in sound event handling. |
...
> **Important:** The actual state transitions (CALM to DISTURBED, DISTURBED to CHASE, etc.) are driven by the C++ perception system. Script cannot force a mind state change --- it only reacts to what die Engine decides.
...
Die Basis input controller (shared with animals) provides override methods:
...
Extends die Basis controller with infected-specific queries:
...
- Andernfalls: full damage via `DamageSystem.CloseCombatDamageName()` with the attack's `m_AmmoType`
...
Zombie perception is primarily handled by the C++ engine. Script exposes das Ergebnis (mind state, target entity) but not the internal perception logic. Jedoch, the script side defines the **target callbacks** and **noise system** that feed into the engine.

### Vision (Visibility Modifiers)

`AITargetCallbacksPlayer` (registered on each `PlayerBase`) provides `GetMaxVisionRangeModifier()`, which tells die Engine how visible a player is:
...
**Vision point selection** also depends on mind state: when the infected is ALERTED or higher, it looks at the player's head bone; andernfalls, it checks the chest (Spine3 bone).
...
There is no script-visible "smell" API. Proximity detection appears to be handled entirely in the C++ engine. Modders should treat die Engine perception as a black box that outputs mind state and target entity.

---

## The Command Handler

`ZombieBase.CommandHandler()` is called every frame by the engine. Es ist der/die/das central decision point:
...
A random roll (0-100) muss <= `stunChance` for the hit animation to play, unless:
...
The Central Economy spawns zombies via `events.xml` using zombie class names and group definitions. This ist nicht script --- it is XML configuration processed by the CE engine:
...
Default hit component: `"Torso"`. Standard hit position component: `"Spine1"`.
...
> **Warning:** `DayZInfectedCommandScript` is NON-MANAGED. Once sent to the CommandHandler via `StartCommand_Script` or `StartCommand_ScriptInst`, die Engine takes ownership. Do not delete it manually while active --- this will cause a crash.
...
5. **Respect the command handler flow** --- use `ModCommandHandlerBefore/Inside/After` stattdessen of overriding `CommandHandler` directly.
6. **Avoid deleting active `DayZInfectedCommandScript`** --- die Engine owns it once started. Let it finish or call `SetFlagFinished(true)`.
...
- Spawnt a zombie with `CreateObjectEx("ZmbF_JournalistNormal_Blue", pos, ECE_PLACE_ON_SURFACE|ECE_INITAI|ECE_EQUIP_ATTACHMENTS)`
...
| Theory | Practice |
|--------|---------|
| Mind states kann set from script | The C++ engine controls transitions; script can only read state and override input controller values |
| Zombies use pathfinding for navigation | Yes, via `AIWorld.FindPath()` and navmesh, but the path planning is internal to die Engine |
...
7. **Deleting zombie entities without `DestroyAIAgent()` first** --- Can cause orphaned AI agents. Die Engine usually handles this, but explicit cleanup is safer for mod-spawned zombies.
8. **Setting leg health to 0 without triggering `m_CrawlTransition`** --- Direct `SetHealth("LeftLeg", "Health", 0)` macht nicht trigger the crawl transition; the logic flows through `EEHitBy` -> `HandleSpecialZoneDamage` -> `EvaluateCrawlTransitionAnimation`.
...
| Aspect | Impact |
|--------|--------|
| **Performance** | Each zombie runs its `CommandHandler` every frame auf dem Server. Large zombie populations (50+) can cause server lag. |
