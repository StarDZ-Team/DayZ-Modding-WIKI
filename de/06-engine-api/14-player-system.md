...
[Home](../../README.md) | [<< Previous: Input System](13-input-system.md) | **Player System**
...
This chapter is an API reference for der Spieler class hierarchy, its identity system, health pools, state checks, equipment access, and the manager objects that drive status effects. All method signatures are taken directly from the vanilla script source.

---

## Class Hierarchy

Der Spieler entity sits at the bottom of a deep inheritance chain. Each level adds capabilities:
...
| `GetFullName()` | `string` | Full name of der Spieler |
...
| `GetPlayer()` | `Man` | Der Spieler entity this identity belongs to |
...
When **Health** reaches 0, der Spieler dies. When **Blood** drops too low, der Spieler loses consciousness and eventually dies. When **Shock** drops to 0, der Spieler goes unconscious.
...
All health modification is **server-authoritative**. Only call these auf dem Server.
...
These events fire auf dem Server and kann overridden in modded classes:
...
Clothing and equipment are attached to named slots on der Spieler entity. Use `FindAttachmentBySlotName()` (defined on `EntityAI`):
...
These operations must run auf dem Server. Calling them auf dem Client will either have no effect or cause desync.
...
### What Syncs Automatisch

PlayerBase registers numerous variables for automatic network synchronization via `RegisterNetSyncVariable*()`. When der Server modifies these variables and calls `SetSynchDirty()`, clients receive updates through `OnVariablesSynchronized()`.

**Automatisch synced variables include:**
...
Dies ist der/die/das client-side callback that fires whenever synced variables change:
...
**Purpose:** Steuert all status effect modifiers (disease, hunger, temperature effects, broken legs).
...
**Purpose:** Verwaltet disease agents (cholera, influenza, salmonella, etc.).

```c
PlayerAgentPool agentPool = player.m_AgentPool;
```

Agents are typischerweise added through contaminated food/water and removed by the immune system or medication.

### BleedingSourcesManagerServer

**Purpose:** Verfolgt individual wound locations and their bleeding rate.
...
Bleeding sources are tied to model bone selections (e.g., `"RightForeArmRoll"`, `"LeftLeg"`, `"RightFoot"`, `"Head"`). On der Client side, `BleedingSourcesManagerRemote` handles particle effects.

### StaminaHandler

**Purpose:** Verwaltet stamina pool, consumption, and recovery.
...
**Purpose:** Verwaltet the shock damage pool and unconsciousness threshold.
...
**Purpose:** Verwaltet the action system (continuous actions like eating, bandaging, crafting).
...
**Purpose:** Behandelt weapon operations (reload, chamber, unjam).
...
**Purpose:** Steuert emote/gesture playback.
...
Dead player entities still exist in die Welt as corpses. Many operations are meaningless or harmful on dead players.
...
Health changes are **server-authoritative**. Setting health auf dem Client wird overwritten by the next server sync, or worse, cause desync.
...
*Dieses Kapitel behandelt the PlayerBase API as of DayZ 1.26. Method signatures are sourced from vanilla script files in `3_Game/` and `4_World/`. For the complete entity hierarchy that PlayerBase inherits from, see [Chapter 6.1: Entity System](01-entity-system.md).*
...
- **Modded Class Conflicts:** Common conflict points are `OnVariablesSynchronized()` (forgetting `super` hides other mods' sync logic), `EEHitBy()` (damage modification mods overriding each other), and der Konstruktor (net sync variable registration order muss consistent).
...
- **Server/Client:** `GetGame().GetPlayer()` returns null on dedicated servers. Use `GetGame().GetPlayers(array)` to iterate server-side players. Manager subsystems like `GetBleedingManagerServer()` return null on clients; use `GetBleedingManagerRemote()` for client-side particle effects stattdessen.
