...
Before you upload anything, go through this list. Skipping items here causes am haeufigsten post-publish headaches.
...
| Folder / File | Purpose |
|---------------|---------|
| `addons/` | Enthaelt all `.pbo` files (packed mod content) and their `.bisign` signature files |
| `keys/` | Enthaelt the public key (`.bikey`) that servers use to verify your PBOs |
...
| **Description** | Detailed overview of what your mod does. Unterstuetzt Steam's BB code formatting (siehe unten) |
...
| Problem | Cause | Fix |
|---------|-------|-----|
| "Addon rejected by server" | Server missing `.bikey`, or `.bisign` macht nicht match `.pbo` | Confirm `.bikey` is in server `keys/` folder. Re-sign PBOs with the correct `.biprivatekey`. |
...
| Conflicts with other mods | Redefining vanilla classes stattdessen of modding them | Use `modded class`, call `super` in overrides, set `requiredAddons` for load order. |
