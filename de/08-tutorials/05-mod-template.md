...
Das **DayZ Mod Template** ist ein Open-Source-Repository, das von InclementDab gepflegt wird und ein vollständiges, sofort verwendbares Mod-Geruest für DayZ bereitstellt:
...
Dies ist der empfohlene Ausgangspunkt für jeden, der bereits einen Hello-World-Mod erstellt hat und zu komplexeren Projekten übergehen möchte.

---

## Was das Template bietet

Das Template enthält alles, was ein DayZ-Mod zum Kompilieren und Laden benötigt:
...
| `.gitignore` | Vorkonfigurierte Ignores für die DayZ-Entwicklung (PBOs, Logs, Temp-Dateien) |

Das Template folgt der standardmäßigen 5-Schichten-Skripthierarchie, die in [Kapitel 2.1: Die 5-Schichten-Skripthierarchie](../02-mod-structure/01-five-layers.md) dokumentiert ist. Alle drei Script-Layer sind in der config.cpp eingebunden, sodass du sofort Code in jeder Schicht platzieren kannst, ohne zusätzliche Konfiguration.
...
**`mod.cpp`** ist der Ausweis deines Mods. Sie steuert, was Spieler in der DayZ-Launcher-Modliste sehen. Siehe [Kapitel 2.3: mod.cpp & Workshop](../02-mod-structure/03-mod-cpp.md) für alle verfügbaren Felder.

**`Scripts/config.cpp`** ist die wichtigste Datei. Sie teilt der DayZ-Engine mit:
- Wovon dein Mod abhängt (`CfgPatches.requiredAddons[]`)
...
**`Scripts/3_Game/`** wird zuerst geladen. Platziere hier Enums, Konstanten, RPC-IDs, Konfigurationsklassen und alles, was keine Weltentitäten referenziert.
...
Das Template wird mit Platzhalternamen ausgeliefert. Du musst diese durch den tatsächlichen Namen deines Mods ersetzen. Hier ist ein systematischer Ansatz.

### Deine Namen wählen

Bevor du Änderungen vornimmst, entscheide dich für:
...
| **Präprozessor-Define** | `MYAWESOMEMOD` | config.cpp defines[], #ifdef-Pruefungen |

### Benennungsregeln

- **Keine Leerzeichen oder Sonderzeichen** in Verzeichnis- und Klassennamen. Verwende PascalCase oder Unterstriche.
- **CfgPatches-Klassennamen müssen global eindeutig sein.** Zwei Mods mit demselben CfgPatches-Klassennamen verursachen Konflikte. Verwende deinen Mod-Namen als Präfix.
...
Öffne `Scripts/config.cpp` und aktualisiere die folgenden Abschnitte.
...
Wenn dein Mod von einem anderen Mod abhängt, füge dessen CfgPatches-Klassennamen zu `requiredAddons[]` hinzu:

```cpp
requiredAddons[] =
{
    "DZ_Data",
    "CF_Scripts"              // Abhängigkeit vom Community Framework
};
```

### CfgMods

Aktualisiere die Mod-Identität und Script-Pfade:
...
**Wichtige Punkte:**
- Der `dir`-Wert muss exakt mit dem Stammordnernamen deines Mods übereinstimmen.
...
Wenn andere Mods die Präsenz deines Mods erkennen sollen, füge ein `defines[]`-Array hinzu:
...
Andere Mods können dann `#ifdef MYAWESOMEMOD` verwenden, um bedingt Code zu kompilieren, der sich mit deinem Mod integriert.

---

## Schritt 5: mod.cpp aktualisieren

Öffne `mod.cpp` im Stammverzeichnis und aktualisiere sie mit den Informationen deines Mods:
...
Benenne innerhalb dieser Ordner alle Platzhalter-`.c`-Dateien um und aktualisiere deren Klassennamen. Wenn das Template zum Beispiel eine Datei wie `ModInit.c` mit einer Klasse namens `ModInit` enthält, benenne sie in `MyAwesomeModInit.c` um und aktualisiere die Klasse:
...
Dies lädt deine Skripte direkt aus den Quellordnern, ohne ein PBO zu packen. Bearbeite eine `.c`-Datei, starte das Spiel neu und sieh die Änderungen sofort.

### Addon Builder verwenden (für die Verteilung)

Wenn du bereit bist zu verteilen:

1. Öffne **DayZ Tools** über Steam
...
Prüfe nach dem Start das Script-Log auf deine Nachrichten:

```
%localappdata%\DayZ\script_<date>_<time>.log
```

Suche nach dem Präfix-Tag deines Mods (z.B. `[MyAwesomeMod]`).
...
DayZ Workbench kann die Skripte deines Mods mit Syntax-Hervorhebung öffnen und bearbeiten:

1. Öffne **Workbench** über DayZ Tools
2. Gehe zu **File > Open** und navigiere zum `Scripts/`-Ordner deines Mods
3. Öffne eine beliebige `.c`-Datei zur Bearbeitung mit grundlegender Enforce-Script-Unterstützung

Workbench liest die `config.cpp`, um zu verstehen, welche Dateien zu welchem Skriptmodul gehören. Daher ist eine korrekt konfigurierte config.cpp essenziell.

### P:-Laufwerk einrichten

Das Template ist dafür konzipiert, vom P:-Laufwerk aus zu arbeiten. Wenn du an einen anderen Ort geklont hast, erstelle eine Verknüpfung:

```batch
mklink /J P:\MyAwesomeMod "D:\Projects\MyAwesomeMod"
```

Dies macht den Mod unter `P:\MyAwesomeMod` zugänglich, ohne Dateien zu verschieben.

### Addon Builder Automatisierung

Für wiederholte Builds kannst du eine Batch-Datei im Stammverzeichnis deines Mods erstellen:
...
| **Alle 3 Script-Layer** | Vorkonfiguriert | Du fügst sie nach Bedarf hinzu |
| **config.cpp** | Vollständig mit allen Modulen | Minimal (nur Mission) |
| **Git-bereit** | .gitignore enthalten | Du erstellst deine eigene |
| **Lernwert** | Niedriger (Dateien vorgefertigt) | Höher (du baust alles selbst) |
| **Empfohlen für** | Erfahrene Modder, neue Projekte | Erstmalige Modder, die die Grundlagen lernen |

**Empfehlung:** Wenn dies dein allererster DayZ-Mod ist, beginne mit [Kapitel 8.1](01-first-mod.md), um jede Datei zu verstehen. Sobald du vertraut bist, verwende das Template für alle zukünftigen Projekte.
...
1. **Ein benutzerdefiniertes Item hinzufügen** -- Folge [Kapitel 8.2: Ein benutzerdefiniertes Item erstellen](02-custom-item.md), um Items in der config.cpp zu definieren.
2. **Ein Admin-Panel bauen** -- Folge [Kapitel 8.3: Ein Admin-Panel bauen](03-admin-panel.md) für Server-Management-UI.
3. **Chat-Befehle hinzufügen** -- Folge [Kapitel 8.4: Chat-Befehle hinzufügen](04-chat-commands.md) für In-Game-Textbefehle.
...
5. **mod.cpp-Optionen lernen** -- Lies [Kapitel 2.3: mod.cpp & Workshop](../02-mod-structure/03-mod-cpp.md) für die Workshop-Veröffentlichung.
6. **Abhängigkeiten hinzufügen** -- Wenn dein Mod das Community Framework oder einen anderen Mod verwendet, aktualisiere `requiredAddons[]` und siehe [Kapitel 2.4: Dein erster Mod](../02-mod-structure/04-minimum-viable-mod.md).

---

**Vorheriges:** [Kapitel 8.4: Chat-Befehle hinzufügen](04-chat-commands.md) | [Startseite](../../README.md)
