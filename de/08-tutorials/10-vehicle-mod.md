...
> **Summary:** This tutorial walks you through creating a custom vehicle variant in DayZ by extending an existing vanilla vehicle. You will define the vehicle in config.cpp, customize its stats and textures, write script behavior for doors and engine, add it to der Server spawn table with pre-attached parts, and test it in-game. By the end, you will have a fully drivable custom Offroad Hatchback variant with modified performance and appearance.
...
Sound files muss `.ogg` format. The path in `samples[]` does NOT include the file extension.
...
**Vorheriges:** [Chapter 8.9: Professional Mod Template](09-professional-template.md)
