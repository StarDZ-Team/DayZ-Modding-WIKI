...
> **Osszefoglalas:** This tutorial teaches you how to debug and test DayZ mods effectively. You will learn to read script logs, use Print debugging, work with DayZDiag and file patching for rapid iteration, diagnose common error messages, and establish a reliable testing workflow. No IDE debugger exists for retail builds -- your tools are script logs, Print statements, Workbench, DayZDiag, and file patching.
...
- [Print Debugging (The Reliable Metodus)](#print-debugging-the-reliable-method)
...
- [Common Error Mintas and Solutions](#common-error-patterns-and-solutions)
...
| Log Entry | Jelentes |
|-----------|---------|
| `SCRIPT (E): Cannot convert string to int` | Tipus mismatch -- passing or assigning the wrong type |
...
| `SCRIPT (E): Metodus 'GetHelth' not found in class 'EntityAI'` | Metodus does not exist -- check spelling and parent class |
...
## Print Debugging (The Reliable Metodus)
...
### Mikor hasznald DayZDiag vs Retail
...
- **Szintaxis highlighting** -- Keywords, types, strings, and comments are color-coded
- **Code completion** -- Tipus a class name followed by a dot to see available methods
- **Error highlighting** -- Szintaxis errors are underlined in red before you run anything
...
| `Metodus 'xyz' not found in class 'Abc'` | Calling a method that does not exist on that class | Check the class hierarchy. Look up the correct method name in the API. |
...
| `Tipus 'MyClass' not found` | The file defining MyClass is not loaded or is in a higher script layer | Check config.cpp script loading order. Lower layers cannot see higher layers. |
...
### Null Pointers and Safe Casting -- Reszleted Peldas
...
Hibas every DayZ modder makes at least once. Learn from them.
...
For RPC/client-server issues, the error is often on one side and a tunet on the other. Check both `%LocalAppData%\DayZ\` (client) and your server's profile folder.
...
- **Always check both client and server logs for RPC issues.** The error is often on one side and a tunet on the other. A server-side null pointer silently drops the response, and the client just sees "no data received."
...
**Kovetkezo:** [Chapter 8.8: Building a HUD Overlay](08-hud-overlay.md)
