...
> **Osszefoglalas:** DayZ is a client-server game. Every line of code you write runs in a specific context -- server, client, or both. Understanding this split is essential for writing secure, functional mods. This chapter explains where code runs, how to detect which side you are on, how to structure multi-package mods, and the patterns that keep server and client code properly separated.
...
| Metodus | Dedicated Server | Client (Remote) | Listen Server |
|--------|:---:|:---:|:---:|
...
**Pelda:** The StarDZ AI client mod uses `type = "mod"` because both server and client need the AI entity class definitions, RPC constants, and sync data structures:
...
**Pelda:** The StarDZ AI Server mod uses `type = "servermod"` because clients should never see the AI brain, perception, combat, or spawning logic:
...
| Layer | Dedicated Server | Client | Listen Server | Megjegyzesek |
|-------|:---:|:---:|:---:|-------|
...
### Mikor hasznald Preprocessor Guards vs Runtime Checks

| Megkozelites | Mikor hasznald | Pelda |
|----------|-------------|---------|
...
### Real Pelda: Client Mission Hook in a Shared Mod
...
## Common Server-Client Mintas

### Minta 1: Server-Side Validation with Client Feedback
...
### Minta 2: Config Sync (Server to Client)
...
**Javitas:** If you need exclusive branches, use `else if` or check `IsDedicatedServer()`:
...
**Javitas:** Use server/client specific subclasses or guard with context checks:
...
- Tipuss that exist only in `MissionGameplay` are available on listen server
...
## Real-World Split Peldas

### Pelda 1: StarDZ AI (Client + Server)
...
### Pelda 3: StarDZ Missions (Client + Server)
...
### Hiba 1: Running Server Logic on Client
...
### Hiba 2: UI Code in Server-Only Mod
...
**Javitas:** All UI code belongs in the client package (`type = "mod"`), wrapped in `#ifndef SERVER`.

### Hiba 3: GetGame().GetPlayer() on Server
...
**Javitas:** On the server, players are passed to you through events, RPCs, or iteration:
...
### Hiba 4: Forgetting Listen Server Compatibility
...
### Hiba 5: Not Using #ifdef for Optional Mod Detection
...
### Hiba 6: Putting Shared Tipuss Only in the Server Package
...
**Javitas:** Shared data structures (RPC data, entity definitions, config classes) go in the client package (`type = "mod"`) so both sides have them:
...
### Hiba 7: Hardcoded Server File Paths on Client
...
**Javitas:** The server loads configs and sends relevant data to clients via RPC. Clients never read server config files directly.
...
**Elozo:** [Chapter 2.5: File Organization Legjobb gyakorlatok](05-file-organization.md)
**Kovetkezo:** [Part 3: GUI & Layout System](../03-gui-system/01-widget-types.md)
