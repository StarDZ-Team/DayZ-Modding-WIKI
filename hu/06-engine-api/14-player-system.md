...
[Fooldal](../../README.md) | [<< Elozo: Input System](13-input-system.md) | **Player System**
...
| Pool | Zone/Tipus | Alapertelmezett Max | What Drains It |
|------|-----------|-------------|----------------|
...
Alapertelmezett maximums are defined in `PlayerKonstansok`:
...
### Where These Metoduss Live

| Metodus | Defined In | Hogyan mukodik |
|--------|-----------|--------------|
| `IsAlive()` | `Object` | Visszaad `!IsDamageDestroyed()` |
| `IsUnconscious()` | `PlayerBase` | Checks command type or `m_IsUnconscious` flag |
| `IsRestrained()` | `PlayerBase` | Visszaad `m_IsRestrained` (synced variable) |
...
## Player Akcios (Server-Side Operations)
...
| Variable | Tipus | What It Represents |
|----------|------|-------------------|
...
| `m_HasBloodTipusVisible` | `bool` | Blood type test done |
...
**Cel:** Controls all status effect modifiers (disease, hunger, temperature effects, broken legs).
...
**Cel:** Manages disease agents (cholera, influenza, salmonella, etc.).
...
**Cel:** Tracks individual wound locations and their bleeding rate.
...
**Cel:** Manages stamina pool, consumption, and recovery.
...
**Cel:** Manages the shock damage pool and unconsciousness threshold.
...
**Cel:** Manages the action system (continuous actions like eating, bandaging, crafting).
...
**Cel:** Handles weapon operations (reload, chamber, unjam).
...
**Cel:** Controls emote/gesture playback.
...
| Manager | Member Variable | Cel |
|---------|----------------|---------|
| `TunetManager` | `m_TunetManager` | Visual/audio symptoms (coughing, sneezing) |
...
### 1. GetGame().GetPlayer() Visszaad Null on Server
...
### 7. Using IsAlive() on Base Object Tipus
...
| Check instance type | `player.GetInstanceTipus()` |
| Restrain | `player.SetRestrained(true)` |

---

*This chapter covers the PlayerBase API as of DayZ 1.26. Metodus signatures are sourced from vanilla script files in `3_Game/` and `4_World/`. For the complete entity hierarchy that PlayerBase inherits from, see [Chapter 6.1: Entity System](01-entity-system.md).*
