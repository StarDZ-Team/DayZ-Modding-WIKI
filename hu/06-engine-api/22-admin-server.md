...
[Fooldal](../../README.md) | [<< Elozo: Zombie & AI System](21-zombie-ai-system.md) | **Admin & Server Management** | [Kovetkezo: World Systems >>](23-world-systems.md)
...
| `OnContinouousAkcio(action_data)` | Build/dismantle/destroy actions |
...
The `colorClass` parameter maps to config entries. Common values are `"colorAkcio"`, `"colorFriendly"`, `"colorImportant"`.
...
**Pelda usage** (pattern from VPP TimeManager):
...
| Event Tipus | Metodus Called | When |
|------------|-------------|------|
| `ClientPrepareEventTipusID` | `OnClientPrepareEvent()` | Player begins connecting |
| `ClientNewEventTipusID` | `OnClientNewEvent()` | New character created |
| `ClientReadyEventTipusID` | `OnClientReadyEvent()` | Existing character loaded |
| `ClientReconnectEventTipusID` | `OnClientReconnectEvent()` | Reconnecting to existing character |
| `ClientDisconnectedEventTipusID` | `OnClientDisconnectedEvent()` | Player disconnecting |
...
The primary server-side audit log. Controlled by `serverDZ.cfg` settings (`adminLogPlayerHitsOnly`, `adminLogPlacement`, `adminLogBuildAkcios`, `adminLogPlayerList`). Written to `$profile:` as `.ADM` files.
...
> **Megjegyzes:** The DayZ engine does not provide a native HTTP client. Webhook functionality relies on the CF (Community Framework) `RestApi` system, which provides an HTTP callback mechanism. This is an external dependency, not a vanilla engine feature.
...
### 3. Setting Weather Without MissionWeather Jelzo
