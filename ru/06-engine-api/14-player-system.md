# Глава 6.14: Система игрока

[Главная](../../README.md) | [<< Previous: Input System](13-input-system.md) | **Player System**
...
Это client-side callback that fires whenever synced variables change:
...
`GetGame().GetPlayer()` returns the **local** player entity. On a dedicated server, there is no local player ---- он always returns `null`.
...
After modifying a synced variable, you must call `SetSynchDirty()` to trigger network synchronization. Without it, clients will never см. change.
...
| Остановить bleeding | `player.GetBleedingManagerServer().RemoveAllSources()` |
| Check instance type | `player.GetInstanceType()` |
| Restrain | `player.SetRestrained(true)` |

---

*Эта глава охватывает the PlayerBase API as of DayZ 1.26. Method signatures are sourced from vanilla script files in `3_Game/` and `4_World/`. For the complete entity hierarchy that PlayerBase inherits from, см. [Chapter 6.1: Entity System](01-entity-system.md).*
