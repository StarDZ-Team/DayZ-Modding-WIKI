...
[Главная](../../README.md) | [<< Previous: Zombie & AI System](21-zombie-ai-system.md) | **Admin & Server Management** | [Next: World Systems >>](23-world-systems.md)
...
| `UnconStart(player)` / `UnconОстановить(player)` | Unconsciousness transitions |
...
The `$profile:` path prefix resolves to the server's profile directory (set via `-profiles=` launch parameter). Это primary location for server-side data:
...
> **Примечание:** The DayZ engine does not provide a native HTTP client. Webhook functionality relies on the CF (Community Framework) `RestApi` system, which provides an HTTP callback mechanism. This is an external dependency, not a vanilla engine feature.
...
The Hive is DayZ's persistence layer ----  native C++ system that stores character data, world objects (tents, barrels, vehicles), and server state to disk. It is not directly accessible from script beyond a small set of `proto native` methods.
