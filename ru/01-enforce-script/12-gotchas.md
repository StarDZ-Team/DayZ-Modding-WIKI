...
- [Полный справочник по ловушкам](#полный-справочник-по-ловушкам)
  ...
  31. [Классы `sealed` нельзя расширять (1.28+)](#31-классы-sealed-нельзя-расширять-128)
  32. [Ограничение параметров метода: максимум 16 (1.28+)](#32-ограничение-параметров-метода-максимум-16-128)
  33. [Предупреждения атрибута `Obsolete` (1.28+)](#33-предупреждения-атрибута-obsolete-128)
  34. [Ошибка сравнения `int.MIN`](#34-ошибка-сравнения-intmin)
  35. [Логическое отрицание элемента массива не работает](#35-логическое-отрицание-элемента-массива-не-работает)
  36. [Сложное выражение при присваивании в массив вызывает краш](#36-сложное-выражение-при-присваивании-в-массив-вызывает-краш)
  37. [`foreach` на возвращаемом значении метода вызывает краш](#37-foreach-на-возвращаемом-значении-метода-вызывает-краш)
  38. [Приоритет побитовых операторов и операторов сравнения](#38-приоритет-побитовых-операторов-и-операторов-сравнения)
  39. [Пустые блоки `#ifdef` / `#ifndef` вызывают краш](#39-пустые-блоки-ifdef--ifndef-вызывают-краш)
  40. [`GetGame().IsClient()` возвращает false при загрузке](#40-getgameisclient-возвращает-false-при-загрузке)
  41. [Сообщения об ошибках компиляции указывают на неправильный файл](#41-сообщения-об-ошибках-компиляции-указывают-на-неправильный-файл)
  42. [Файлы `crash_*.log` --- это не краши](#42-файлы-crashlog----это-не-краши)
