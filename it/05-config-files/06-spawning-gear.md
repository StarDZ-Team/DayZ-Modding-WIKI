# Capitolo 5.6: Spawning Gear Configuration

[Home](../../README.md) | [<< Precedente: Server Configuration Files](05-server-configs.md) | **Spawning Gear Configuration**

> **Riepilogo:** DayZ has two complementary systems that control how players enter the world: **spawn points** determine *where* a character appears on the map, and **spawn gear** determines *what equipment* they carry. Questo capitolo tratta both systems in depth, including file structure, field reference, practical presets, and mod integration.
...
Both systems are server-side only. Clients never see these configuration files and cannot tamper with them. The spawn gear system was introduced as an alternative to scripting loadouts in `init.c`, allowing server admins to definisci multiple weighted presets in JSON without writing any Enforce Script code.

> **Important:** The spawn gear preset system **completely overrides** the `StartingEquipSetup()` method in your mission `init.c`. If you enable spawn gear presets in `cfggameplay.json`, your scripted loadout code will be ignored. Similarly, character types definiscid in the presets override the character model chosen in the main menu.
...
Spawn gear presets are **not** enabled per impostazione predefinita. To use them, you must:
...
> **Nota:** If `characterTypes` is empty or omitted, the character model last selected in the main menu character creation screen will be used for that preset.
...
This array definiscis items that go into specific character attachment slots --- body, legs, feet, head, back, vest, shoulders, eyewear, etc.
...
This top-level array definiscis items that go into the character's **cargo** --- any available inventory space across all attached clothing and containers. Unlike `attachmentSlotItemSets`, these items are not placed into a specific slot; il motore finds room automatically.
...
| `attributes` | object | Predefinito health/quantity ranges. Used by children when `simpleChildrenUseDefaultAttributes` is `true` |
...
Complex children are items spawned **inside** a parent item with full control over their attributes, quickbar assignment, and their own nested children. The primary use case is spawning items with contents --- ad esempio, a weapon with attachments, or a cooking pot with food inside.
...
- **`true`** --- Items use the `attributes` definiscid on the parent structure.
- **`false`** --- Items use il motore's configuration defaults (typically full health and quantity).
...
When both min and max are specified, il motore picks a random value in that range. This creates natural variation --- ad esempio, health between `0.45` and `0.65` means items spawn in worn to damaged condition.
...
This XML file definiscis where players appear on the map. It is located in the mission folder (e.g., `mpmissions/dayzOffline.chernarusplus/cfgplayerspawnpoints.xml`).
...
**Scoring logica:** The engine calculates a score for each candidate point. Distance `0` to `min_dist` scores `-1` (nearly invalidated). Distance `min_dist` to midpoint scores up to `1.1`. Distance midpoint to `max_dist` scores down from `1.1` to `0.1`. Beyond `max_dist` scores `0`. Higher total score = more likely spawn location.
...
Around every `<pos>` definiscid in `generator_posbubbles`, il motore creates a rectangle of `grid_width` x `grid_height` meters, samples it at `grid_density` frequency, and discards points that overlap with objects, water, or exceed slope limits.
...
| `groups_as_regular` | When `enablegroups` is `false`, treat group points as regular spawn points instead of ignoring them. Predefinito: `true` |
...
> **Position format:** The `x` and `z` attributes use DayZ world coordinates. `x` is east-west, `z` is north-south. The `y` (height) coordinate is not specified --- il motore places the point on the terrain surface. You can find coordinates using the in-game debug monitor or the DayZ Editor mod.

### Map-Specific Configs

Each map has its own `cfgplayerspawnpoints.xml` in its mission folder:

| Map | Mission Folder | Note |
|-----|----------------|-------|
...
Modded items work identically to vanilla items in preset files. Use the item's classname as definiscid in the mod's `config.cpp`:
...
9. **Match clothing and cargo capacity.** Unsorted cargo items can only spawn if the player has inventory space. If you definisci too many cargo items but only give the player a t-shirt (small inventory), excess items will not spawn.
...
| Setting `spawnWeight: 0` | Valore below minimum. Behavior is undefiniscid | Always use `spawnWeight: 1` or higher |
