...
[Home](../../README.md) | [<< Precedente: Dialogs & Modals](08-dialogs-modals.md) | **Real Mod UI Patterns** | [Successivo: Advanced Widgets >>](10-advanced-widgets.md)
...
1. **JMRenderableModuleBase** -- Declares the module's metadata (title, icon, layout path, permissions). Manages the CF_Window lifecycle. Does not contain UI logica.
...
Expansion definiscis six notification visual types, each with its own layout:
...
Expansion wraps ScriptView in a menu classe base that handles input locking, blur effects, and update timers:
...
This is set via the `scriptclass` property in the layout file. Il motore chiama `OnWidgetScriptInit` automatically when `CreateWidgets()` processes a widget with a script class.
...
## Riepilogo: Which Pattern to Use When
...
| Admin panel system | Modulo + Form + Window (module registration pattern) | COT |
...
| Notification system | Tipo enum + per-type layout + static creation API | Expansion |
