# Capitolo 6.13: Input System

[Home](../../README.md) | [<< Precedente: Action System](12-action-system.md) | **Input System**
...
1. **inputs.xml** (config layer) --- declares named actions, assigns default keybindings, and organizes them into groups for the player's Controls settings menu. Vedi [Capitolo 5.2: inputs.xml](../05-config-files/02-inputs-xml.md) for full coverage.

2. **UAInput API** (script layer) --- queries input state at runtime. This is what your scripts call ogni frame to detect presses, releases, holds, and analog valori.

Questo capitolo tratta the script layer: the classes, methods, and patterns you use to read and control inputs from Enforce Script.
...
- **`LocalHold()`** does NOT fire immediately. It waits for il motore's hold threshold to be met first. Use `LocalPress()` if you need instant response.
...
- `PlayerBase.CommandHandler()` --- runs ogni frame for the local player
...
| Funzionalita | UAInput (GetUApi) | OnKeyPress |
|---------|-------------------|------------|
...
The `KeyCode` enum is definiscid in `1_Core/proto/ensystem.c`. These constants are used with `OnKeyPress()`, `OnKeyRelease()`, `KeyState()`, and `DisableKey()`.
...
The mission system provides named exclude groups definiscid in il motore's `specific.xml`. When activated, they disable categories of inputs:
...
Expansion uses its own custom exclude group registered with il motore:
...
1. At startup, il motore reads all `inputs.xml` files from loaded mods
...
If you definiscid a Ctrl+Key combo in inputs.xml, the UAInput system handles it automatically. But if you need to check modifier state manually alongside a UAInput:
...
The engine method is `Supress()` with a single 's' (not `Suppress`). The global method `SupressNextFrame()` also uses a single 's'. This is a quirk of il motore API:
...
*Questo capitolo tratta the script-side Input System API. For the XML configuration that registers keybindings, see [Chapter 5.2: inputs.xml](../05-config-files/02-inputs-xml.md).*
...
- **Check inputs only in per-frame callbacks like `OnUpdate()`.** `LocalPress()` returns true for exactly one frame. Checking it in event handlers or callbacks that do not run ogni frame will miss key presses.
...
- **Performance:** Input polling is lightweight. `GetUApi().GetInputByName()` performs a hash lookup. Caching the `UAInput` riferimento in a member variable avoids repeated lookups but is not strictly necessary for performance.
