# Capitolo 6.14: Player System

[Home](../../README.md) | [<< Precedente: Input System](13-input-system.md) | **Player System**

---

## Introduzione

`PlayerBase` is the single most important class nel modding di DayZ. Every gameplay system --- health, hunger, bleeding, stamina, inventory, restraints, unconsciousness --- lives on the player entity or one of its manager subsystems. Whether you are writing an admin tool, a survival mechanic, or a PvP mod, you will interact with PlayerBase constantly.
...
| Method | Return Type | Descrizione |
|--------|-------------|-------------|
...
| Pool | Zone/Type | Predefinito Max | What Drains It |
|------|-----------|-------------|----------------|
...
These events fire on the server and puo essere sovrascritto in modded classes:
...
Default maximums are definiscid in `PlayerConstants`:

| Stat | Costante | Predefinito |
|------|----------|---------|
...
Clothing and equipment are attached to named slots on the player entity. Use `FindAttachmentBySlotName()` (definiscid on `EntityAI`):
...
| Variabile | Tipo | What It Represents |
|----------|------|-------------------|
...
`PlayerIdentity` and `PlayerBase` are separate objects linked by il motore:
...
*Questo capitolo tratta the PlayerBase API as of DayZ 1.26. Method signatures are sourced from vanilla script files in `3_Game/` and `4_World/`. For the complete entity hierarchy that PlayerBase inherits from, see [Chapter 6.1: Entity System](01-entity-system.md).*
...
- **Modded Class Conflicts:** Common conflict points are `OnVariablesSynchronized()` (forgetting `super` hides other mods' sync logica), `EEHitBy()` (damage modification mods overriding each other), and the constructor (net sync variable registration order must be consistent).
