# Capitolo 6.22: Admin & Server Management

[Home](../../README.md) | [<< Precedente: Zombie & AI System](21-zombie-ai-system.md) | **Admin & Server Management** | [Successivo: World Systems >>](23-world-systems.md)
...
DayZ does not expose a direct `KickPlayer()` script function. Instead, il motore provides `DisconnectPlayer()` which terminates the connection.
...
The `EClientKicked` enum (from `3_Game/global/errormodulehandler/clientkickedmodule.c`) definiscis all possible kick reasons il motore recognizes:
...
These are the primary hook points for admin mods. Both COT and VPP use `modded class MissionServer` to inject admin logica at each stage.
...
> **Nota:** The DayZ engine does not provide a native HTTP client. Webhook functionality relies on the CF (Community Framework) `RestApi` system, which provides an HTTP callback mechanism. This is an external dependency, not a vanilla engine feature.
...
- **Call `MissionWeather(true)` before locking weather valori.** Without this flag, il motore's automatic weather controller will override your settings when it computes the next forecast change.
...
If you set weather values without calling `MissionWeather(true)`, il motore's automatic weather controller will override your changes at the next forecast computation.
...
- **RPC ID Collisions:** COT uses CF's string-based RPC routing. VPP uses `GetRPCManager().AddRPC()` with string identifiers. Custom admin mods should avoid using raw integer RPC IDs that may collide with vanilla `ERPCs` valori.
...
- **Server/Client:** All admin commands are server-authoritative. Client mods provide UI only. Server-only mods (`-servermod=`) can implement admin logica without distributing scripts to clients, but cannot provide in-game UI.
