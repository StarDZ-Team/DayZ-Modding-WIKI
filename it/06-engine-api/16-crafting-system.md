# Capitolo 6.16: Crafting System

[Home](../../README.md) | [<< Precedente: Sound System](15-sound-system.md) | **Crafting System**
...
| `m_ResultUseSoftSkills[i]` | `false` | Allow soft skills to modify result valori. |
...
Sovrascrivi `RegisterRecipies()` in a modded `PluginRecipesManagerBase`:
...
Called from the `RecipeBase` constructor. Set all field values here. Do not call `super.Init()` --- it is an empty declaration in the classe base.
...
Called during `CheckRecipe()` after the built-in condition checks (`CheckConditions()`) pass. Use this for custom validation logica that cannot be expressed through the field-based conditions. The `ingredients[]` array is **sorted** --- index 0 maps to your ingredient slot 0 definition, index 1 to slot 1.
...
Called after `SpawnItems()` and `ApplyModifications*()` have run. This is where you implement custom post-craft logica --- transferring properties, modifying state, triggering effects. The built-in field system handles most modifications automatically; `Do()` is for anything beyond that.
...
Sovrascrivi to return `true` if the recipe should continue repeating after completion (like repair recipes where you keep repairing until the item is fixed or the tool runs out):
...
- `uid` --- Sovrascrivi the crafting animation when this specific ingredient is used. For example, `CleanWeapon` uses `CMD_ACTIONFB_CLEANING_WEAPON` when the ingredient is a `DefaultWeapon`.
...
When you insert a classe genitore name like `"Inventory_Base"` or `"DefaultWeapon"`, the recipe matches **all subclasses**. The engine uses `g_Game.IsKindOf()` for matching, which walks the config hierarchy:
...
For each result definiscid by `AddResult()`:
...
2. **GenerateRecipeCache()** --- walks all items in `CfgVehicles`, `CfgWeapons`, and `CfgMagazines`. For each item, resolves its full config hierarchy and inherits recipe cache entries from classe genitorees.
...
If the quantity reduction brings the item to zero or below, il motore destroys it automatically (via `AddQuantity()` return value).
...
Recipes like `DisinfectItem` and `PurifyWater` modify an ingredient in place without producing a new item. They definisci no `AddResult()` and use `Do()` to alter the ingredient:
...
2. **Use inheritance-based ingredients wisely.** Inserting `"Inventory_Base"` matches every item in the game. This is powerful but can create unexpected recipe conflicts. Prefer specific class names or intermediate classe basees.
...
5. **Keep `CanDo()` lightweight.** It runs frequently during recipe validation (ogni frame the player looks at a craftable combination). Avoid heavy operations like config lookups or iteration.
...
7. **Prefer `m_IngredientAddHealth` over `m_IngredientSetHealth`.** Adding negative health is relative and works across different item types with different max health valori. Setting absolute health can produce unexpected results.
...
| Categoria | Count | Pattern | Esempio |
|----------|-------|---------|---------|
...
- **The `m_ResultToInventory >= 0` swap path is commented out.** The code for swapping result position with an ingredient exists in `SpawnItems()` but the swap logica is inside a commented block. Only `-2` (ground) and `-1` (inventory) work.
...
- `GetValidRecipes()` runs ogni frame on the client when the player holds an item and looks at another item. The cache makes this fast (O(1) lookup per item pair, then O(n) for matched recipes where n is small).
...
| File | Scopo |
|------|---------|
| `4_World/classes/recipes/recipebase.c` | `RecipeBase` --- classe base, field definitions, execution pipeline |
