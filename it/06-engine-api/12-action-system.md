# Capitolo 6.12: Action System

[Home](../../README.md) | [<< Precedente: Central Economy](10-central-economy.md) | **Action System**

---

## Introduzione

The Action System is how DayZ handles all player interactions with items and the world. Every time a player eats food, opens a door, bandages a wound, repairs a wall, or turns on a flashlight, il motore runs through the action pipeline. Understanding this pipeline --- from condition checks to animation callbacks to server execution --- is fundamental to creating any interactive gameplay mod.

The system lives primarily in `4_World/classes/useractionscomponent/` and is built around three pillars:

1. **Action classes** that definisci what happens (logica, conditions, animations)
...
Questo capitolo tratta the full API, class hierarchy, lifecycle, and practical patterns for creating custom actions.
...
|----------|-----------|------------|----------|
| Categoria constant | `AC_SINGLE_USE` | `AC_CONTINUOUS` | `AC_INTERACT` |
...
The action state machine uses these constants definiscid in `3_Game/constants.c`:

| Costante | Valore | Significato |
|----------|-------|---------|
| `UA_NONE` | 0 | No action running |
| `UA_PROCESSING` | 2 | Azione in progress |
| `UA_FINISHED` | 4 | Azione completed successfully |
| `UA_CANCEL` | 5 | Azione cancelled by player |
| `UA_INTERRUPT` | 6 | Azione interrupted externally |
...
These methods are called in order during an action's lifetime. Sovrascrivi them in your custom actions:

| Method | Called on | Scopo |
|--------|-----------|---------|
...
| `OnStart()` | Both | Azione begins (cancels placing if active) |
| `OnStartServer()` | Server | Server-side start logica |
| `OnStartClient()` | Client | Client-side start effects |
| `OnExecute()` | Both | Animation event fired --- main execution |
| `OnExecuteServer()` | Server | Server-side execution logica |
...
| `OnEnd()` | Both | Azione finished (success or cancel) |
...
You can extend `ActionData` for custom data. Sovrascrivi `CreateActionData()` in your action:
...
3. For each candidate, il motore calls `GetActions()` on the object to find registered actions
...
| Costante | Valore (meters) | Typical use |
|----------|---------------|-------------|
...
Actions are registered on entities through the `SetActions()` / `AddAction()` / `RemoveAction()` pattern. Il motore chiama `GetActions()` on an entity to retrieve its action list; the first time this happens, `InitializeActions()` builds the map via `SetActions()`.

### On ItemBase (Inventory Items)

The most common pattern. Sovrascrivi `SetActions()` in a `modded class`:
...
4. At runtime, il motore queries all actions for the matching input type
...
- `ActionConsume` (the parent) handles all the food consumption logica
...
Sovrascrivi the corresponding methods in your action to enable these:
...
When `HasProneException()` returns true, il motore uses `m_CommandUIDProne` instead of `m_CommandUID` if the player is in prone stance.
...
|----------|-------|-------------|
| `AC_UNCATEGORIZED` | 0 | Predefinito --- should not be used |
...
Remember: `SetActions()` is called per **class type**, not per instance. If you add an action in a classe genitore, all children inherit it. If you only want the action on specific subclasses, either:
- Add it only in the specific subclass's `SetActions()`
- Add a type check in `ActionCondition()` as a guard

### 8. Forgetting HasTarget() Sovrascrivi
...
Without this, il motore expects a target object and may not show the action, or will try to sync a non-existent target to the server.
...
1. **Choose your classe base**: `ActionSingleUseBase` for instant, `ActionContinuousBase` for timed, `ActionInteractBase` for world toggles
...
4. **Implement server logica** in `OnExecuteServer()` or `OnFinishProgressServer()`
...
The system is designed to be modular: condition components handle "can this happen?", action components handle "how long does it take?", and your overrides handle "what does it do?". Keep server logica on the server, visual feedback on the client, and always null-check your targets.
...
- **Put all state-changing logica in `OnExecuteServer` or `OnFinishProgressServer`.** Health changes, item deletion, and inventory manipulation must run server-side. `OnExecuteClient` is only for visual feedback.
...
- **Multi-Mod:** Actions are registered per class type via `SetActions()`. Two mods adding different actions to the same item class both work -- actions accumulate. Tuttavia, if both mods override `SetActions()` without calling `super`, only the last-loaded mod's actions survive.
- **Performance:** `ActionCondition()` is evaluated ogni frame for every candidate action on the player's current target. Keep it lightweight -- avoid expensive raycasts, config lookups, or array iterations inside condition checks.
- **Server/Client:** The action pipeline is split: condition checks and UI display run on the client, execution callbacks run on the server. The engine handles synchronization via internal RPCs. Never rely on client-side state for authoritative game logica.
