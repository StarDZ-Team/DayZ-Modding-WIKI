# Capitolo 6.21: Zombie & AI System

[Home](../../README.md) | [<< Precedente: Particle Effects](20-particle-effects.md) | **Zombie & AI System**
...
Questo capitolo tratta the full class hierarchy, the mind state machine, movement and attack commands, the perception/targeting system, spawning patterns, and modding hooks. All method signatures and constants are taken directly from the vanilla script source. Where behavior is driven by the C++ engine with no script-visible API, that is noted explicitly.
...
| `DayZInfected` | `3_Game/entities/dayzinfected.c` | Comando starters (`StartCommand_Move`, `_Attack`, `_Death`, etc.), `EEHitBy` |
...
| `ZombieBase` | `4_World/entities/creatures/infected/zombiebase.c` | `CommandHandler`, fight logica, crawl transition, damage hit evaluation, net sync |
...
> **Important:** The actual state transitions (CALM to DISTURBED, DISTURBED to CHASE, etc.) are driven by the C++ perception system. Script cannot force a mind state change --- it only reacts to what il motore decides.
...
| Method | Signature | Descrizione |
|--------|-----------|-------------|
| `SovrascriviMovementSpeed` | `void SovrascriviMovementSpeed(bool state, float movementSpeed)` | Force a specific movement speed |
| `GetMovementSpeed` | `float GetMovementSpeed()` | Current movement speed |
| `SovrascriviHeading` | `void SovrascriviHeading(bool state, float heading)` | Force heading direction |
| `SovrascriviTurnSpeed` | `void SovrascriviTurnSpeed(bool state, float turnSpeed)` | Force turn rate |
| `SovrascriviAlertLevel` | `void SovrascriviAlertLevel(bool state, bool alerted, int level, float inLevel)` | Force alert level |
| `SovrascriviBehaviourSlot` | `void SovrascriviBehaviourSlot(bool state, int slot)` | Force behaviour slot |
...
`DayZInfectedType.RegisterAttacks()` definiscis two attack groups with parameters read from config:
...
Zombie perception is primarily handled by the C++ engine. Script exposes the result (mind state, target entity) but not the internal perception logica. Tuttavia, the script side definiscis the **target callbacks** and **noise system** that feed into il motore.

### Vision (Visibility Modifiers)

`AITargetCallbacksPlayer` (registered on each `PlayerBase`) provides `GetMaxVisionRangeModifier()`, which tells il motore how visible a player is:
...
There is no script-visible "smell" API. Proximity detection appears to be handled entirely in the C++ engine. Modders should treat il motore perception as a black box that outputs mind state and target entity.

---

## The Command Handler

`ZombieBase.CommandHandler()` is called ogni frame by il motore. It is the central decision point:
...
| Hook | When | Usage |
|------|------|-------|
| `ModCommandHandlerBefore` | Before any vanilla logica | Return `true` to skip all default behavior |
| `ModCommandHandlerInside` | After death/move handling, before combat | Return `true` to skip combat logica |
| `ModCommandHandlerAfter` | After all vanilla logica | Return `true` (no practical effect, runs last) |
...
- **Torso / Head**: Sets `m_HeavyHitSovrascrivi = true` (forces heavy hit animation)
...
`DayZInfectedType.RegisterHitComponentsForAI()` definiscis which body parts zombies target and with what probability:
...
> **Warning:** `DayZInfectedCommandScript` is NON-MANAGED. Once sent to the CommandHandler via `StartCommand_Script` or `StartCommand_ScriptInst`, il motore takes ownership. Do not delete it manually while active --- this will cause a crash.
...
6. **Avoid deleting active `DayZInfectedCommandScript`** --- il motore owns it once started. Let it finish or call `SetFlagFinished(true)`.
...
- Sovrascrivis movement speed via `GetInputController().SovrascriviMovementSpeed(true, speed)`
...
| Zombies use pathfinding for navigation | Yes, via `AIWorld.FindPath()` and navmesh, but the path planning is internal to il motore |
...
6. **Not checking `!IsAlive()` before AI operations** --- Dead zombies still exist as entities but their AI state is undefiniscid.
...
8. **Setting leg health to 0 without triggering `m_CrawlTransition`** --- Direct `SetHealth("LeftLeg", "Health", 0)` does not trigger the crawl transition; the logica flows through `EEHitBy` -> `HandleSpecialZoneDamage` -> `EvaluateCrawlTransitionAnimation`.
...
| Aspect | Impact |
|--------|--------|
| **Performance** | Each zombie runs its `CommandHandler` ogni frame on the server. Large zombie populations (50+) can cause server lag. |
...
| **Client/Server** | Comando handler, fight logica, and damage run server-side only. Sound events and animation playback are client-side. |
