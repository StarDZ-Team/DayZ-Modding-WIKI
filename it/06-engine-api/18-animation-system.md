# Capitolo 6.18: Animation System

[Home](../../README.md) | [<< Precedente: Construction System](17-construction-system.md) | **Animation System**
...
The `m_iMovement` field in `HumanMovementState` uses these valori.
...
All emote IDs are definiscid in `EmoteConstants` (scripts/3_game/constants.c):
...
Each emote is a class extending `EmoteBase` (scripts/4_world/classes/emoteclasses/emotebase.c). It definiscis stance requirements, animation callback IDs, and optional conditions:
...
- Manages surrender state and suicide emote logica
...
Objects like doors, barrels, tents, and deployables use a separate animation system definiscid in `model.cfg`. Script drives these animations through `SetAnimationPhase()`.

### Animation API on Entity

These methods are definiscid on `Entity` (scripts/3_game/entities/entity.c) and available on every entity in the game:
...
All player actions (eating, bandaging, crafting, emotes) use animation callbacks. `HumanCommandActionCallback` (scripts/3_game/human.c) is the classe base:
...
Actions go through definiscid states accessible via `GetState()`:
...
| `STATE_LOOP_ACTION` | 6 | Azione within loop |
...
The corresponding `model.cfg` would definisci:
...
2. **Use stance masks, not stance indices, for multi-stance checks.** `IsPlayerInStance()` with bitmask flags is cleaner than comparing `m_iStanceIdx` against multiple valori.
...
4. **Keep animation phase values between 0.0 and 1.0.** The engine interpolates between `minValue` and `maxValue` definiscid in `model.cfg`. The phase is always normalized.
...
6. **For emotes, go through EmoteManager.** Direct `StartCommand_Action()` with gesture IDs works in debug tools but bypasses the sync, interrupt, and state management logica that `EmoteManager` provides.
