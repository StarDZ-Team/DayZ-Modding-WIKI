# Capitolo 4.8: Building Modeling -- Doors & Ladders

[<< Precedente: Workbench Guide](07-workbench-guide.md) | **Building Modeling**

---

## Introduzione

Buildings in DayZ are more than static scenery. Players interact with them constantly -- opening doors, climbing ladders, taking cover behind walls. Creating a custom building that supports these interactions requires careful model setup: doors need rotation axes and named selections across multiple LODs, ladders need precisely placed climbing paths definiscid entirely through Memory LOD vertices.

Questo capitolo tratta the complete workflow for adding interactive doors and climbable ladders to custom building models, based on official Bohemia Interactive documentation.
...
Interactive doors require three things to come together: the P3D model with correctly named selections and memory points, a `model.cfg` that definiscis the animation skeleton and rotation parameters, and a `config.cpp` game config that links the door to sounds, damage zones, and game logica.
...
2. **Memory LOD vertices** that definisci:
   - **Rotation axis** -- Two vertices forming the axis of rotation, assigned to a named selection like `door1_axis`. This axis definiscis the hinge line around which the door pivots.
...
Almost all doors in vanilla DayZ are **120 x 220 cm** (width x height). Using these standard dimensions ensures animations look correct and characters fit through openings naturally. Model your doors **closed per impostazione predefinita** and animate them to the open position -- Bohemia plans to support doors opening in both directions in the future.

### model.cfg -- Skeletons and Animations

Any animated door requires a `model.cfg` file. This config definiscis the bone structure (skeleton) and the animation parameters. Place `model.cfg` near your model file, or higher in the folder structure -- the exact location is flexible as long as the binarizer can find it.
...
| `memory` | Set to `1` to indicate the axis is definiscid in the Memory LOD. |
| `minValue` / `maxValue` | Animation phase range. Typically `0` to `1`. |
| `angle0` / `angle1` | Rotation angles in **radians**. `angle1` definiscis how far the door opens. Use negative values to reverse direction. A value of `1.4` radians is approximately 80 degrees. |
...
The game config connects the animated model to game systems -- sounds, damage, and door state logica. The config class name **must** follow the pattern `land_modelname` to link correctly with the model.
...
- Each class name under `class Doors` must correspond to the `source` parameter definiscid in `model.cfg`.
...
- Similarly, definisci only **one** entry in `DamageZones` for the double door pair.

### Porte Scorrevoli

For doors that slide along a track rather than swinging (such as barn doors or sliding panels), change the animation `type` in `model.cfg` from `"rotation"` to `"translation"`. The axis vertices in the Memory LOD then definisci the direction of travel instead of the pivot line.
...
The ladder is definiscid entirely by named vertices in the Memory LOD. Every selection name begins with `ladderN_` where **N** is the ladder ID, starting from `1`. A building can have multiple ladders (`ladder1_`, `ladder2_`, `ladder3_`, etc.).
...
Unlike the Arma series, DayZ does **not** require a `ladders[]` array in the game config class. Tuttavia, two things are still necessary:

1. Your model must have a **config representation** -- a `config.cpp` with a `CfgVehicles` class (the same classe base used for doors; see the door config section above).
2. The **Geometry LOD** must contain the named property `class` with the value `house`.

Beyond these two requirements, the ladder is fully definiscid by the Memory LOD vertices and the View Geometry selection. No `model.cfg` animation entries are needed.
...
### Consistenza delle Selezioni Nominate
...
1. **Model doors closed per impostazione predefinita.** Animate from closed to open. Bohemia plans to support opening doors in both directions, so starting from closed is future-proof.
...
4. **Sovrascrivi bounding spheres for large buildings.** If your building has doors that swing outward significantly, create a Memory LOD selection covering the full animated extent and link it with the `bounding` config parameter.
...
7. **Keep one `model.cfg` per model or del tuo mod.** The `model.cfg` does not need to sit next to the `.p3d` file, but keeping them close makes organization easier. It can also be placed higher in the folder structure to cover multiple models.
...
| Axis vertices missing or only one vertex definiscid. | Door pivots from the wrong point or does not rotate at all. | Place exactly two vertices in the Memory LOD for the axis selection (e.g., `door1_axis`). |
| `source` in `model.cfg` does not match class name in `config.cpp` Doors. | Door is not linked to game logica -- no sounds, no state changes. | Ensure the `source` parameter and the Doors class name are identical. |
