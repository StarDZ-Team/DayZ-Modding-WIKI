# Capitolo 2.6: Server vs Client Architecture

> **Riepilogo:** DayZ is a client-server game. Every line of code you write runs in a specific context -- server, client, or both. Understanding this split is essential for writing secure, functional mods. This chapter explains where code runs, how to detect which side you are on, how to structure multi-package mods, and the patterns that keep server and client code properly separated.
...
DayZ uses a **dedicated server** model. The server and the client are separate processes running separate executables. They communicate over the network, and il motore handles synchronization of entities, variables, and RPCs.
...
The dedicated server is a **headless process**. It has no window, no graphics card output, no monitor, no keyboard, no mouse. It exists only to run game logica.
...
**Esempio:** The StarDZ AI client mod uses `type = "mod"` because both server and client need the AI entity class definitions, RPC constants, and sync data structures:
...
**When to use:** Server-side logica that clients should never have access to. This includes:
- Spawn algorithms (prevents players from predicting loot)
- AI brain logica (prevents exploit analysis)
- Admin commands and server management
- Database connections and external API calls
- Anti-cheat validation logica

**Esempio:** The StarDZ AI Server mod uses `type = "servermod"` because clients should never see the AI brain, perception, combat, or spawning logica:
...
If your spawn logica is in a `type = "mod"` package, **every player downloads it**. They can decompile the PBO and read your spawn algorithms, loot tables, admin passwords, or anti-cheat logica. Always put sensitive server logica in a `type = "servermod"` package.

---

## The config.cpp type Field

Inside `config.cpp` (in the `CfgMods` section), there is also a `type` field. This one controls how il motore treats the mod internally:
...
The `config.cpp` also contains the `definiscis[]` array, which is how you enable preprocessor symbols for cross-mod detection:
...
Notice that the server mod definiscis both `STARDZ_AI` and `STARDZ_AISERVER`. This allows server-side code to detect whether just the client mod is present or the full server package is loaded.
...
A single mod folder with `type = "mod"` ships everything to clients. For many mods, this is fine. But for mods with sensitive server logica, you need to split:
...
- Manager/controller classes (spawn logica, AI brains)
...
This ensures the client package compiles first, and the server package can reference all types definiscid in the client package.
...
| Layer | Dedicated Server | Client | Listen Server | Note |
|-------|:---:|:---:|:---:|-------|
...
The engine automatically definiscis `SERVER` when compiling for a dedicated server. This is a **compile-time** check, not a runtime check:
...
### Quando Usare Preprocessor Guards vs Runtime Checks

| Approccio | When to Use | Esempio |
|----------|-------------|---------|
...
| `GetGame().IsServer()` | Runtime branching within code that runs on both sides | Entity update logica that differs per side |
| `GetGame().IsClient()` | Runtime branching within code that runs on both sides | Playing effects only on client |

### Real Esempio: Client Mission Hook in a Shared Mod
...
Una trappola comune: you test your mod on a listen server, everything works, you publish it, and it crashes on every dedicated server. This happens because:
...
3. Entity class definitions from the client package are available to server logica

### definiscis[] for Optional Dependency Detection

The `definiscis[]` array in `CfgMods` creates preprocessor symbols that other mods can check with `#ifdef`:
...
- The bulk of the logica is server-side and invisible to players
...
- [ ] All server logica (spawning, validation, AI brains) is in the server package
...
- [ ] `definiscis[]` array matches between `mod.cpp` and `config.cpp`
...
**Successivo:** [Part 3: GUI & Layout System](../03-gui-system/01-widget-types.md)
