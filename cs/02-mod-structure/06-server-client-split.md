...
> **Shrnuti:** DayZ is a client-server game. Every line of code you write runs in a specific context -- server, client, or both. Understanding this split is essential for writing secure, functional mods. This chapter explains where code runs, how to detect which side you are on, how to structure multi-package mods, and the patterns that keep server and client code properly separated.
...
**Priklad:** The StarDZ AI client mod uses `type = "mod"` because both server and client need the AI entity class definitions, RPC constants, and sync data structures:
...
**Priklad:** The StarDZ AI Server mod uses `type = "servermod"` because clients should never see the AI brain, perception, combat, or spawning logic:
...
| Layer | Dedicated Server | Client | Listen Server | Poznamky |
|-------|:---:|:---:|:---:|-------|
...
### Kdy pouzit Preprocessor Guards vs Runtime Checks

| Pristup | Kdy pouzit | Priklad |
|----------|-------------|---------|
...
### Real Priklad: Client Mission Hook in a Shared Mod
...
## Common Server-Client Vzors

### Vzor 1: Server-Side Validation with Client Feedback
...
### Vzor 2: Config Sync (Server to Client)
...
**Oprava:** If you need exclusive branches, use `else if` or check `IsDedicatedServer()`:
...
**Oprava:** Use server/client specific subclasses or guard with context checks:
...
## Real-World Split Priklads

### Priklad 1: StarDZ AI (Client + Server)
...
### Priklad 3: StarDZ Missions (Client + Server)
...
### Chyba 1: Running Server Logic on Client
...
### Chyba 2: UI Code in Server-Only Mod
...
**Oprava:** All UI code belongs in the client package (`type = "mod"`), wrapped in `#ifndef SERVER`.

### Chyba 3: GetGame().GetPlayer() on Server
...
**Oprava:** On the server, players are passed to you through events, RPCs, or iteration:
...
### Chyba 4: Forgetting Listen Server Compatibility
...
### Chyba 5: Not Using #ifdef for Optional Mod Detection
...
### Chyba 6: Putting Shared Typs Only in the Server Package
...
**Oprava:** Shared data structures (RPC data, entity definitions, config classes) go in the client package (`type = "mod"`) so both sides have them:
...
### Chyba 7: Hardcoded Server File Paths on Client
...
**Oprava:** The server loads configs and sends relevant data to clients via RPC. Clients never read server config files directly.
...
**Predchozi:** [Chapter 2.5: File Organization Doporucene postupy](05-file-organization.md)
**Dalsi:** [Part 3: GUI & Layout System](../03-gui-system/01-widget-types.md)
