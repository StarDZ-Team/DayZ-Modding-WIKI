...
[Domu](../../README.md) | [<< Predchozi: Particle Effects](20-particle-effects.md) | **Zombie & AI System**
...
### GameKonstanty (AI-Related)

From `3_Game/constants.c`:

| Konstanta | Hodnota | Ucel |
|----------|-------|---------|
...
The `HandleMindStateZmena` method in `ZombieBase` reads the mind state from the input controller each frame and triggers idle animation transitions:
...
> **Dulezite:** The actual state transitions (CALM to DISTURBED, DISTURBED to CHASE, etc.) are driven by the C++ perception system. Script cannot force a mind state change --- it only reacts to what the engine decides.
...
| `IsTurning` | `bool IsTurning()` | Vraci true during turn animation |
...
- `cfgVehicles <ZombieClass> AttackAkces AttackShort ammoTyp` (light/fight)
- `cfgVehicles <ZombieClass> AttackAkces AttackLong ammoTyp` (heavy/fight)
- `cfgVehicles <ZombieClass> AttackAkces AttackRun ammoTyp` (chase)

### Attack Cooldown

Fight attacks use a cooldown timer decremented by `pDt * GameKonstanty.AI_ATTACKSPEED` (1.5x speed). Chase attacks have no cooldown gating in script --- they fire as soon as target alignment is valid.
...
### Vision (Visibility Modifikators)

`AITargetCallbacksPlayer` (registered on each `PlayerBase`) provides `GetMaxVisionRangeModifikator()`, which tells the engine how visible a player is:

**Speed modifiers** (from `PlayerKonstanty`):
...
**Speed noise** (from `PlayerKonstanty`):
...
**ECE Priznaks** (from `3_Game/ce/centraleconomy.c`):

| Priznak | Hodnota | Ucel |
|------|-------|---------|
...
Vychozi hit component: `"Torso"`. Vychozi hit position component: `"Spine1"`.
...
> **Varovani:** `DayZInfectedCommandScript` is NON-MANAGED. Once sent to the CommandHandler via `StartCommand_Script` or `StartCommand_ScriptInst`, the engine takes ownership. Do not delete it manually while active --- this will cause a crash.
...
6. **Avoid deleting active `DayZInfectedCommandScript`** --- the engine owns it once started. Let it finish or call `SetPriznakFinished(true)`.
...
| **Network** | Three synced variables per zombie (`m_MindState`, `m_MovementSpeed`, `m_IsCrawling`, `m_OrientationSynced`). Zmenas trigger `SetSynchDirty()`. |
