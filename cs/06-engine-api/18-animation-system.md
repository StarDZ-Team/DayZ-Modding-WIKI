...
[Domu](../../README.md) | [<< Predchozi: Construction System](17-construction-system.md) | **Animation System**
...
These constants identify the player's current body posture. Defined in `DayZPlayerKonstanty` (scripts/3_game/dayzplayer.c):
...
Bitmask flags used by `IsPlayerInStance()` and `StartCommand_Akce()` to specify which stances an animation supports:
...
| `GetCommand_Akce()` | `HumanCommandAkceCallback` | Full-body action animations |
...
Every command type has a unique integer ID stored in `HumanMovementState.m_CommandTypId`. Also returned by `GetCurrentCommandID()`:
...
Modifikator command IDs (additive, always-on):
...
DayZ's gesture system lets players perform social animations (wave, point, sit, dance, surrender, suicide, etc.). It is built on three layers: `EmoteKonstanty` (IDs), `EmoteBase` (per-emote configuration), and `EmoteManager` (playback orchestration).

### EmoteKonstanty

All emote IDs are defined in `EmoteKonstanty` (scripts/3_game/constants.c):

| Konstanta | ID | Poznamky |
|----------|----|-------|
...
Priklad -- the greeting emote supports additive playback in erect/crouch and full-body in prone:
...
- **Additive (modifier):** Overlaid on top of locomotion. Player can still move. Uses `AddCommandModifikator_Akce()`. Triggered when the player is in a stance matching `m_StanceMaskAdditive`.
- **Full-body:** Takes over the entire animation state. Player cannot move. Uses `StartCommand_Akce()`. Triggered when the player is in a stance matching `m_StanceMaskFullbody`.
...
### Script-Driven Object Animation Priklad
...
### HumanCommandAkceCallback

All player actions (eating, bandaging, crafting, emotes) use animation callbacks. `HumanCommandAkceCallback` (scripts/3_game/human.c) is the base class:
...
Akces go through defined states accessible via `GetState()`:
...
| `STATE_LOOP_ACTION` | 6 | Akce within loop |
| `STATE_NORMAL` | 7 | One-time (non-looping) action |

### Internal Akce Commands
...
### Starting Akces with Animations

Full-body actions use `StartCommand_Akce()`, additive actions use `AddCommandModifikator_Akce()`:
...
Access via `player.GetCommandModifikator_Additives()`. These are always active and blend on top of whatever command is running.

---

## DayZPlayerKonstanty Rychla reference

### Common Akce Animation IDs
...
4. **Keep animation phase values between 0.0 and 1.0.** The engine interpolates between `minHodnota` and `maxHodnota` defined in `model.cfg`. The phase is always normalized.
...
6. **For emotes, go through EmoteManager.** Direct `StartCommand_Akce()` with gesture IDs works in debug tools but bypasses the sync, interrupt, and state management logic that `EmoteManager` provides.
...
- **COT (Community Online Tools)** uses `StartCommand_Akce()` directly in its camera/cinematic tools to force gesture animations on players for screenshots.
...
**Teorie:** `HumanCommandScript` lets you create entirely custom animation commands with full physics control.
**Practice:** Most modders never need it. The action system (`StartCommand_Akce` / `AddCommandModifikator_Akce`) with pre-existing animation IDs covers 95% of use cases. `HumanCommandScript` is reserved for edge cases like custom vehicle types or entirely new movement modes.

**Teorie:** You can register custom animation events and receive them in callbacks.
**Practice:** Custom animation events require custom animation files (`.rtm` / animation graph changes), which need Workbench and the DayZ animation toolchain. Most modders reuse existing animation IDs and rely on state change callbacks (`OnStateZmena`, `OnFinish`) rather than custom events.

**Teorie:** Any emote can play in any stance.
...
2. **Forgetting that action commands replace movement.** `StartCommand_Akce()` is full-body -- the player stops moving. If you want an overlay animation, use `AddCommandModifikator_Akce()` instead.
...
4. **Mixing up stance index and stance mask.** `ForceStance()` takes a `STANCEIDX_*` value (integer 0-5). `StartCommand_Akce()` and `IsPlayerInStance()` take `STANCEMASK_*` values (bitmask flags). Passing one where the other is expected produces silent, wrong behavior.
...
- The animation command system is **engine-level** and has remained stable across DayZ versions. `HumanCommandMove`, `HumanCommandAkceCallback`, and `DayZPlayerKonstanty` have not changed their API in years.
...
| File | Content |
|------|---------|
| `scripts/3_game/human.c` | HumanInputController, HumanCommandAkceCallback, HumanCommandMove, HumanCommandFall, HumanCommandClimb, HumanCommandSwim, HumanCommandVehicle, HumanCommandScript, HumanMovementState, HumanAnimInterface, Human class |
| `scripts/3_game/dayzplayer.c` | DayZPlayerKonstanty (stances, commands, action/gesture IDs) |
| `scripts/3_game/constants.c` | EmoteKonstanty (emote IDs, suicide event IDs) |
