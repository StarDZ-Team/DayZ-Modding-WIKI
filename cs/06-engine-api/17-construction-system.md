...
[Domu](../../README.md) | [<< Predchozi: Crafting System](16-crafting-system.md) | **Construction System**
...
Akce type constants (defined in `_constants.c`):
...
Full-body continuous action. Duration: `UATimeSpent.BASEBUILDING_CONSTRUCT_MEDIUM`. Requires non-ruined tool in hand. Uses the **variant system** --- `ConstructionAkceData.OnUpdateAkces()` populates a list of buildable parts for the targeted component, and `m_VariantID` selects which part to build.
...
On completion, calls `BuildPartServer()` and damages the tool (`UADamageApplied.BUILD`). Both `AkceConditionContinue()` and `OnFinishProgressServer()` perform collision checks via `IsCollidingEx()` to prevent building through players or geometry.
...
The `OnUpdateAkces()` callback is registered with `AkceVariantManager` and fires when the player looks at a construction object, populating the build part list for the action variant system.

---

## Tvorba vlastnich stavitelnych objektu
...
- **Component names** in geometry LOD matching part names (for `GetAkceComponentName()`)
...
## Vanilla stavitelne objekty

### Fence

The most common structure. Kit: `FenceKit`. Funkces a gate system with three states (`GATE_STATE_NONE`, `GATE_STATE_PARTIAL`, `GATE_STATE_FULL`). A fully constructed gate can accept a `CombinationLock` attachment. Supports barbed wire (2 slots creating area damage when mounted) and camo net. Gate opening rotates 100 degrees over ~2 seconds.

### Watchtower

Multi-level tower. Kit: `WatchtowerKit`. Up to 3 floors with walls and roofs per level. Konstanty: `MAX_WATCHTOWER_FLOORS = 3`, `MAX_WATCHTOWER_WALLS = 3`. Has a `MAX_FLOOR_VERTICAL_DISTANCE = 0.5` check preventing attachment of items from too far below the target floor.

### ShelterSite

Temporary construction site. Kit: `ShelterKit`. Three mutually exclusive build options (leather, fabric, stick) that each spawn a different finished `ShelterBase` subclass and delete the site. Built without tools via `AkceBuildShelter`.

---

## Poskozeni a napadani

Each construction part maps to a **damage zone** whose name matches the part name (case-insensitive). When a part is built, its health is set to maximum. When a damage zone reaches `STATE_RUINED`, `EEHealthLevelZmenad()` triggers automatic destruction:
...
- **Raid mods** override `EEHealthLevelZmenad()` or adjust `DamageZones` hitpoints to tune raid difficulty.
