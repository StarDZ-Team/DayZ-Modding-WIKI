...
[Domu](../../README.md) | [<< Predchozi: Input System](13-input-system.md) | **Player System**
...
| Pool | Zone/Typ | Vychozi Max | What Drains It |
|------|-----------|-------------|----------------|
...
Vychozi maximums are defined in `PlayerKonstanty`:
...
| Metoda | Defined In | Jak to funguje |
|--------|-----------|--------------|
| `IsAlive()` | `Object` | Vraci `!IsDamageDestroyed()` |
| `IsUnconscious()` | `PlayerBase` | Checks command type or `m_IsUnconscious` flag |
| `IsRestrained()` | `PlayerBase` | Vraci `m_IsRestrained` (synced variable) |
...
## Player Akces (Server-Side Operations)
...
**Ucel:** Controls all status effect modifiers (disease, hunger, temperature effects, broken legs).
...
**Ucel:** Manages disease agents (cholera, influenza, salmonella, etc.).
...
**Ucel:** Tracks individual wound locations and their bleeding rate.
...
**Ucel:** Manages stamina pool, consumption, and recovery.
...
**Ucel:** Manages the shock damage pool and unconsciousness threshold.
...
**Ucel:** Manages the action system (continuous actions like eating, bandaging, crafting).
...
**Ucel:** Handles weapon operations (reload, chamber, unjam).
...
**Ucel:** Controls emote/gesture playback.
...
### 1. GetGame().GetPlayer() Vraci Null on Server
