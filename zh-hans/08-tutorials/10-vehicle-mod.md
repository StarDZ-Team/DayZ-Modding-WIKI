...
> **摘要:** This tutorial walks you through creating a custom vehicle variant in DayZ by extending an existing vanilla vehicle. You will define the vehicle in config.cpp, customize its stats and textures, write script behavior for doors and engine, add it to the server spawn table with pre-attached parts, and test it in-game. By the end, you will have a fully drivable custom Offroad Hatchback variant with modified performance and appearance.
...
Vehicle definitions live in `CfgVehicles`, just like items. Despite the class name, `CfgVehicles` holds everything -- items, buildings, and actual vehicles alike. The key difference for vehicles is the 父类 and the additional configuration for damage zones, attachments, and simulation parameters.
...
| 字段 | 用途 |
|-------|---------|
| `scope = 2` | Makes the vehicle spawnable. Use `0` for 基类es that should never spawn directly. |
| `displayName` | Name shown in admin tools and in-game. You can use `$STR_` references for localization. |
| `requiredAddons[]` | Must include `"DZ_Vehicles_Wheeled"` so the 父类 `OffroadHatchback` is loaded before your class. |
...
Other vanilla vehicle 父类es you could extend:
...
| `rpmRedline` | Engine redline RPM. Beyond this, 引擎 bounces off the rev limiter. |
...
The `componentNames[]` must match named components in the vehicle's geometry LOD. Since we inherit the Niva model, we use placeholder names here -- the 父类's geometry components are what actually matter for collision detection. If you are using the vanilla model without modification, the parent's component mapping applies automatically.
...
Your `Scripts/config.cpp` must register the `4_World` layer so 引擎 loads your script:
...
**CrewCanGetThrough** -- This is called 每帧 to determine if a player can enter or exit a seat. The Niva's rear seats (indices 2 and 3) work differently from the front seats: the front seatback must be folded forward (animation phase > 0.5) before rear passengers can get through. This matches the real-world behavior of a 2-door hatchback where rear passengers must tilt the front seat.
...
Vehicles spawn as empty shells 默认情况下 -- no wheels, doors, or engine parts. To make them spawn with parts pre-attached, add entries to `cfgspawnabletypes.xml` in the server mission folder:
...
| **Engine starts** | Get in, hold 引擎 start key. Listen for start sound. |
...
- **Use `scope = 0` for internal 基类es.** If you create an abstract base vehicle that other variants extend, set `scope = 0` so it never spawns directly.
...
- **Test door logic thoroughly.** Enter/exit every seat, open/close every door, try accessing 引擎 bay with the hood closed. CrewCanGetThrough bugs are the most common vehicle mod issue.
...
| 概念 | 理论 | 现实 |
|---------|--------|---------|
| `SimulationModule` in config.cpp | Full control over vehicle physics | Not all parameters override cleanly when extending a 父类. If your speed/torque changes seem to have no effect, try adjusting `transmissionRatio` and gear `ratios[]` instead of just `torqueMax`. |
...
| Custom textures via hidden selections | Swap any texture freely | Only selections the model author marked as "hidden" 可以被重写. If you need to retexture a part not in `hiddenSelections[]`, you must create a new model or modify the existing one in Object Builder. |
...
| Engine sounds | Set any SoundSet name | Sound sets must be defined in `CfgSoundSets` somewhere in the loaded configs. If you reference a sound set that does not exist, 引擎 silently falls back to no sound -- no error in the log. |
...
**下一章:** Expand your vehicle mod with custom door models, interior textures, or even a completely new vehicle body using Blender and Object Builder.
...
The physics geometry is not loading. This usually means `requiredAddons[]` is missing `"DZ_Vehicles_Wheeled"`, so the 父类 physics config is not inherited.
...
Verify that `GetCarDoorsState()` uses the correct slot names. The Niva uses `"NivaDriverDoors"`, `"NivaCoDriverDoors"`, `"NivaHood"`, and `"NivaTrunk"`. These 必须完全匹配, including capitalization.
...
Engine sounds are assigned in the constructor. If you misspell a SoundSet name (例如 `"offroad_engine_Start_SoundSet"` instead of `"offroad_engine_start_SoundSet"`), 引擎 silently uses no sound. Sound set names are case-sensitive.
