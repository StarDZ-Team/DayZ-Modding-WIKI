...
Publishing to the Steam Workshop is the final step in the DayZ modding journey. Everything you have learned in previous chapters culminates here. Once your mod is on the Workshop, any DayZ player can subscribe, download, and play with it. 本章涵盖 the complete process: preparing your mod, signing PBOs, uploading, and maintaining updates.
...
| Folder / File | 用途 |
|---------------|---------|
| `addons/` | 包含 all `.pbo` files (packed mod content) and their `.bisign` signature files |
| `keys/` | 包含 the public key (`.bikey`) that servers use to verify your PBOs |
...
你也可以使用 the `DSCreateKey` tool directly from a terminal:
...
| Component | When to Increment | 示例 |
|-----------|-------------------|---------|
...
6. **常见问题** -- common questions answered preemptively
