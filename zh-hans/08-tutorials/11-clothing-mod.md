# 第 8.11: Creating Custom Clothing

> **摘要:** This tutorial walks you through creating a custom tactical jacket for DayZ. You will choose a 基类, define the clothing in config.cpp with insulation and cargo properties, retexture it with a camo pattern using hidden selections, add localization and spawning, and optionally extend it with scripted behavior. By the end, you will have a wearable jacket that keeps players warm, holds items, and spawns in the world.
...
Clothing in DayZ inherits from `Clothing_Base`, but you almost never extend that directly. DayZ provides intermediate 基类es for each body slot:

| 基类 | Body Slot | Examples |
|------------|-----------|----------|
...
To find 基类es, look in `scripts/4_world/entities/itembase/clothing_base.c` (defines all slot bases) and `scripts/4_world/entities/itembase/clothing/` (one file per clothing family).
...
**Health levels:** Each entry is `{ healthThreshold, { materialPath } }`. When health drops below a threshold, 引擎 swaps the material. Vanilla damage rvmats add wear marks and tears.
...
**Important:** Always call `super` at the start of every override. The 父类 handles critical engine behavior.
...
3. **Can you equip it?** Should go to Body slot. If not, check the 父类 chain.
...
| Missing `requiredAddons` | Config parser cannot resolve 父类 | Include `"DZ_Characters_Tops"` for tops |
...
- How DayZ clothing inherits from slot-specific 基类es (`Top_Base`, `Pants_Base`, etc.)
...
**下一章:** Apply the same techniques to create pants (`Pants_Base`), boots (`Shoes_Base`), or a vest (`Vest_Base`). The config structure is identical -- only the 父类 and inventory slot change.
