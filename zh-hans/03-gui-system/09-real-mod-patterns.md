...
[Home](../../README.md) | [<< 上一章: Dialogs & Modals](08-dialogs-modals.md) | **Real Mod UI Patterns** | [下一章: Advanced Widgets >>](10-advanced-widgets.md)
...
Expansion wraps ScriptView in a menu 基类 that handles input locking, blur effects, and update timers:
...
This is set via the `scriptclass` property in the layout file. 引擎调用 `OnWidgetScriptInit` automatically when `CreateWidgets()` processes a widget with a script class.
...
| Admin panel system | 模块 + Form + Window (module registration pattern) | COT |
...
| Notification system | 类型 enum + per-type layout + static creation API | Expansion |
| Toolbar command system | `EditorCommand` + `EditorCommandManager` + shortcuts | DayZ Editor |
| 菜单 bar with items | `EditorMenu` + `ObservableCollection<EditorMenuItem>` | DayZ Editor |
