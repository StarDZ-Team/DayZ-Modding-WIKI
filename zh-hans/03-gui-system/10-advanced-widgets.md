...
[Home](../../README.md) | [<< 上一章: Real Mod Patterns](09-real-mod-patterns.md) | **Advanced Widgets**
...
本章涵盖 every advanced widget type with confirmed API signatures extracted from vanilla source code and real mod usage.
...
`CanvasWidget` is immediate-mode: you must `Clear()` and redraw 每帧. This is typically done in an `Update()` or `OnUpdate()` callback.
...
- **Clear and redraw 每帧.** `CanvasWidget` does not retain state between frames in most use cases where the view changes (camera movement, etc.). Call `Clear()` at the start of each update.
...
### 用法 Pattern -- Inventory Character Preview
...
The rotation and zoom patterns are identical to `ItemPreviewWidget` -- use `SetModelOrientation()` with mouse drag, and `SetSize()` with mouse wheel. 参见 previous section for the full code.
...
2. **Clear canvas 每帧.** Always call `Clear()` before redrawing. Failing to clear causes drawings to accumulate and creates visual artifacts.
