# 第 6.19: Terrain & World Queries

[Home](../../README.md) | [<< 上一章: Animation System](18-animation-system.md) | **Terrain & World Queries**
...
There is no single `SurfaceIsWater` function in 引擎. To check for any water, combine both:
...
**返回：** `true` if the ray hit something.
...
- **Never raycast 每帧 without throttling.** Even `RaycastRV` is expensive at scale. Use timers (0.1--0.5 second intervals) for periodic checks. The rangefinder uses a 0.5-second timer for its measurements.
...
- **Performance Impact:** `GetObjectsAtPosition` with large radii is the most common performance mistake. A mod that calls it 每帧 with a 50m+ radius will cause noticeable server lag. Raycast operations are cheaper but still should not run 每帧 on many entities.
...
- **WorldData Subclassing:** If your mod needs to read or override temperature or weather data, note that `WorldData` is subclassed per map. Modding the 基类 affects all maps; modding `ChernarusPlusData` only affects Chernarus.
...
| 错误 | 修复方法 |
|---------|-----|
| Calling `GetObjectsAtPosition` 每帧 with a large radius | Use a timer (0.25--1.0 second interval). Cache the results array. |
