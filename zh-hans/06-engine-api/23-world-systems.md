...
[<< 上一章: Admin & Server Management](22-admin-server.md) | **World Systems**
...
本章涵盖 five mission-folder configuration systems:
...
- The PPE effect tints the player's vision (green/yellow tint 默认情况下)
...
| 类型 | Breadcrumbs? | EyeAccommodation | 用途 |
|------|-------------|-------------------|---------|
...
| **Inner** | Empty array | `< 1.0` (typically `0.0`) | 常量 darkness deep underground. Eye accommodation is fixed at the configured value |
...
| `Radius` | float | Influence radius in meters. Set to `-1` for 引擎 default |
...
> **提示:** When using `UseRaycast: 1`, raise the breadcrumb position slightly off the floor (a few centimeters in the Y axis) to avoid the ray being blocked by the ground surface.
...
示例： `"0xffbb0a1e"` = fully opaque dark red. The value is not case-sensitive.
...
| 错误 | Consequence | 修复方法 |
|---------|-------------|-----|
| JSON comments (`//` or `/* */`) in config files | 文件 fails to parse; features silently disabled | Remove all comments from production JSON files |
