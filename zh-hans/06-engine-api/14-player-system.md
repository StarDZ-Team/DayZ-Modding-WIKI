...
| Pool | Zone/Type | 默认值 Max | What Drains It |
|------|-----------|-------------|----------------|
...
These events fire on the server and 可以被重写 in modded classes:
...
`PlayerIdentity` and `PlayerBase` are separate objects linked by 引擎:
...
*本章涵盖 the PlayerBase API as of DayZ 1.26. Method signatures are sourced from vanilla script files in `3_Game/` and `4_World/`. For the complete entity hierarchy that PlayerBase inherits from, see [Chapter 6.1: Entity System](01-entity-system.md).*
