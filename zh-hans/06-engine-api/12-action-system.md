# 第 6.12: Action System

[Home](../../README.md) | [<< 上一章: Central Economy](10-central-economy.md) | **Action System**

---

## 简介

The Action System is how DayZ handles all player interactions with items and the world. Every time a player eats food, opens a door, bandages a wound, repairs a wall, or turns on a flashlight, 引擎 runs through the action pipeline. Understanding this pipeline --- from condition checks to animation callbacks to server execution --- is fundamental to creating any interactive gameplay mod.
...
本章涵盖 the full API, class hierarchy, lifecycle, and practical patterns for creating custom actions.
...
|----------|-----------|------------|----------|
| 类别 constant | `AC_SINGLE_USE` | `AC_CONTINUOUS` | `AC_INTERACT` |
...
| `UA_PROCESSING` | 2 | 操作 in progress |
| `UA_FINISHED` | 4 | 操作 completed successfully |
| `UA_CANCEL` | 5 | 操作 cancelled by player |
| `UA_INTERRUPT` | 6 | 操作 interrupted externally |
...
| `OnStart()` | Both | 操作 begins (cancels placing if active) |
...
| `OnEnd()` | Both | 操作 finished (success or cancel) |
...
3. For each candidate, 引擎 calls `GetActions()` on the object to find registered actions
...
Actions are registered on entities through the `SetActions()` / `AddAction()` / `RemoveAction()` pattern. 引擎调用 `GetActions()` on an entity to retrieve its action list; the first time this happens, `InitializeActions()` builds the map via `SetActions()`.
...
4. At runtime, 引擎 queries all actions for the matching input type
...
When `HasProneException()` returns true, 引擎 uses `m_CommandUIDProne` instead of `m_CommandUID` if the player is in prone stance.
...
| 常量 | 值 | 说明 |
|----------|-------|-------------|
| `AC_UNCATEGORIZED` | 0 | 默认值 --- should not be used |
...
Remember: `SetActions()` is called per **class type**, not per instance. If you add an action in a 父类, all children inherit it. If you only want the action on specific subclasses, either:
...
Without this, 引擎 expects a target object and may not show the action, or will try to sync a non-existent target to the server.
...
1. **Choose your 基类**: `ActionSingleUseBase` for instant, `ActionContinuousBase` for timed, `ActionInteractBase` for world toggles
...
- **Multi-Mod:** Actions are registered per class type via `SetActions()`. Two mods adding different actions to the same item class both work -- actions accumulate. 但是, if both mods override `SetActions()` without calling `super`, only the last-loaded mod's actions survive.
- **Performance:** `ActionCondition()` is evaluated 每帧 for every candidate action on the player's current target. Keep it lightweight -- avoid expensive raycasts, config lookups, or array iterations inside condition checks.
