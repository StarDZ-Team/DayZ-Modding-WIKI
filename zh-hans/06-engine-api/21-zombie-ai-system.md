...
[Home](../../README.md) | [<< 上一章: Particle Effects](20-particle-effects.md) | **Zombie & AI System**
...
| `DayZInfected` | `3_Game/entities/dayzinfected.c` | 命令 starters (`StartCommand_Move`, `_Attack`, `_Death`, etc.), `EEHitBy` |
...
> **Important:** The actual state transitions (CALM to DISTURBED, DISTURBED to CHASE, etc.) are driven by the C++ perception system. Script cannot force a mind state change --- it only reacts to what 引擎 decides.
...
Zombie perception is primarily handled by the C++ engine. Script exposes the result (mind state, target entity) but not the internal perception logic. 但是, the script side defines the **target callbacks** and **noise system** that feed into 引擎.

### Vision (Visibility Modifiers)

`AITargetCallbacksPlayer` (registered on each `PlayerBase`) provides `GetMaxVisionRangeModifier()`, which tells 引擎 how visible a player is:
...
There is no script-visible "smell" API. Proximity detection appears to be handled entirely in the C++ engine. Modders should treat 引擎 perception as a black box that outputs mind state and target entity.

---

## The Command Handler

`ZombieBase.CommandHandler()` is called 每帧 by 引擎. It is the central decision point:
...
> **Warning:** `DayZInfectedCommandScript` is NON-MANAGED. Once sent to the CommandHandler via `StartCommand_Script` or `StartCommand_ScriptInst`, 引擎 takes ownership. Do not delete it manually while active --- this will cause a crash.
...
6. **Avoid deleting active `DayZInfectedCommandScript`** --- 引擎 owns it once started. Let it finish or call `SetFlagFinished(true)`.
...
| Zombies use pathfinding for navigation | Yes, via `AIWorld.FindPath()` and navmesh, but the path planning is internal to 引擎 |
...
| Aspect | Impact |
|--------|--------|
| **Performance** | Each zombie runs its `CommandHandler` 每帧 on the server. Large zombie populations (50+) can cause server lag. |
...
| **Client/Server** | 命令 handler, fight logic, and damage run server-side only. Sound events and animation playback are client-side. |
