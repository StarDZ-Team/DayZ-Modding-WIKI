...
Loaded only when `enableCfgGameplayFile = 1` in `serverDZ.cfg`. Without this, 引擎 uses hardcoded defaults.
...
If your mod introduces a new 基类 not inheriting from `Inventory_Base`, `DefaultWeapon`, or `DefaultMagazine`, add it as a `rootclass`. The `act` attribute specifies entity type: `character` for AI, `car` for vehicles.
...
| `cfggameplay.json` controls all gameplay | All tuning goes here | Many gameplay values are hardcoded in script or config.cpp and 不能被重写 by cfggameplay.json |
