# Capítulo 5.5: Arquivos de Configuração do Servidor

[Home](../../README.md) | [<< Anterior: ImageSet Format](04-imagesets.md) | **Arquivos de Configuração do Servidor** | [Próximo: Configuração de Equipamento de Spawn >>](06-spawning-gear.md)

> **Resumo:** DayZ servers are configured through XML, JSON, and script files in the mission folder (e.g., `mpmissions/dayzOffline.chernarusplus/`). These files control item spawns, economy behavior, gameplay rules, and server identity. Understanding them is essential for adding custom items to the loot economy, tuning server parameters, or building a custom mission.
...
- [globals.xml --- Economy Parâmetros](#globalsxml--economy-parameters)
...
Controla what spawns **already attached to or inside** other items.
...
Valors range from `0.0` (pristine) to `1.0` (ruined).
...
| `verifyAssinaturas` | Assinatura check level (2 = strict) |
...
If your mod introduces a new base class not inheriting from `Inventory_Base`, `PadrãoWeapon`, or `PadrãoMagazine`, add it as a `rootclass`. The `act` attribute specifies entity type: `character` for AI, `car` for vehicles.
...
Controla which CE subsystems are active:
