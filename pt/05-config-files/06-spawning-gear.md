...
- [Practical Exemplos](#practical-examples)
  - [Padrão Survivor Loadout](#default-survivor-loadout)
...
1. **Onde does the character appear?** --- Controlled by `cfgplayerspawnpoints.xml`.
...
| System | File | Format | Controla |
|--------|------|--------|----------|
| Spawn Points | `cfgplayerspawnpoints.xml` | XML | **Onde** --- map positions, distance scoring, spawn groups |
...
| `simpleChildrenUsePadrãoAttributes` | bool | Whether simple children inherit these attributes |
| `simpleChildrenTypes` | array | Item classnames to spawn inside this item |

Exemplo --- a weapon with attachments and magazine:
...
Controla how the grid of candidate spawn points is generated around each position bubble:
...
| `groups_as_regular` | When `enablegroups` is `false`, treat group points as regular spawn points instead of ignoring them. Padrão: `true` |
