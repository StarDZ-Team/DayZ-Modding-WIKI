...
[Home](../../README.md) | [<< Anterior: Sistema de Ações](12-action-system.md) | **Sistema de Input**
...
1. **inputs.xml** (config layer) --- declares named actions, assigns default keybindings, and organizes them into groups for the player's Controla settings menu. See [Capítulo 5.2: inputs.xml](../05-config-files/02-inputs-xml.md) for full coverage.
...
## Reading Input State --- UAInput Métodos
...
## Reading Input State --- Input Class Métodos
...
## Input Query Métodos Reference

### UAInput Métodos

| Método | Retorna | Quando Verdadeiro | Caso de Uso |
|--------|---------|-----------|----------|
...
| `LocalValor()` | `float` | Always (returns current value) | Mouse axes, gamepad triggers, analog input |

### Input Class Métodos

| Método | Retorna | Assinatura | Equivalent UAInput Método |
|--------|---------|-----------|--------------------------|
...
| `LocalValor()` | `float` | `LocalValor(string action, bool check_focus = true)` | `UAInput.LocalValor()` |

### Important Timing Notas
...
### Onde Else Can You Check Inputs?
...
| Appears in Controla menu | Yes | No |
...
| Modificadors | `KC_LSHIFT`, `KC_RSHIFT`, `KC_LCONTROL`, `KC_RCONTROL`, `KC_LMENU` (left Alt), `KC_RMENU` (right Alt) |
...
**Important:** Always call `GetUApi().UpdateControla()` after modifying input states in bulk.
...
Método signatures on the `Mission` class:
...
The `bForceSupress` parameter on `RemoveActiveInputExcludes` calls `SupressPróximoFrame` internally to prevent input bleed when re-enabling.
...
3. Padrão key bindings from `<preset>` are applied (unless the player has customized them)
...
For complete inputs.xml syntax, see [Capítulo 5.2: inputs.xml](../05-config-files/02-inputs-xml.md).
...
This is rarely used. The inputs.xml approach is preferred because it integrates properly with the Controla settings menu and preset system.
...
### Using OnKeyPress for Player-Facing Açãos
...
### Forgetting UpdateControla After Bulk Changes

When you `ForceDisable()` multiple inputs, you must call `UpdateControla()` for the changes to take effect:
...
The engine method is `Supress()` with a single 's' (not `Suppress`). The global method `SupressPróximoFrame()` also uses a single 's'. This is a quirk of the engine API:
...
*This chapter covers the script-side Sistema de Input API. For the XML configuration that registers keybindings, see [Capítulo 5.2: inputs.xml](../05-config-files/02-inputs-xml.md).*

---

## Boas Práticas

- **Always use `UAInput` via inputs.xml for player-facing keybindings.** This allows players to rebind keys, shows actions in the Controla menu, and supports gamepad input. Reserve `OnKeyPress` for debug shortcuts only.
...
- **Call `GetUApi().UpdateControla()` after bulk `ForceDisable()` changes.** Without this flush call, disable/enable state changes may not take effect until the next frame, causing one-frame input bleed.
- **Remember that `Supress()` uses a single "s".** The engine API spells it `Supress()` and `SupressPróximoFrame()`. Using the correct English spelling `Suppress` will not compile.
