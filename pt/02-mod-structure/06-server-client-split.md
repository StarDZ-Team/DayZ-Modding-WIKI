# Capítulo 2.6: Arquitetura Servidor vs Cliente

> **Resumo:** DayZ is a client-server game. Every line of code you write runs in a specific context -- server, client, or both. Understanding this split is essential for writing secure, functional mods. This chapter explains where code runs, how to detect which side you are on, how to structure multi-package mods, and the patterns that keep server and client code properly separated.
...
- [Checking Onde Your Code Runs](#checking-where-your-code-runs)
- [The mod.cpp type Campo](#the-modcpp-type-field)
- [The config.cpp type Campo](#the-configcpp-type-field)
- [Multi-Package Mod Architecture](#multi-package-mod-architecture)
- [The Golden Rules](#the-golden-rules)
- [Script Camada and Side Matrix](#script-layer-and-side-matrix)
- [Preprocessor Guards](#preprocessor-guards)
- [Common Server-Client Padrãos](#common-server-client-patterns)
- [Listen Server Gotchas](#listen-server-gotchas)
- [Dependency Between Split Mods](#dependency-between-split-mods)
- [Real-World Split Exemplos](#real-world-split-examples)
- [Erros Comuns](#common-mistakes)
- [Decision Flowchart](#decision-flowchart)
- [Resumo Checklist](#summary-checklist)
...
**Exemplo:** The StarDZ AI client mod uses `type = "mod"` because both server and client need the AI entity class definitions, RPC constants, and sync data structures:
...
**Exemplo:** The StarDZ AI Server mod uses `type = "servermod"` because clients should never see the AI brain, perception, combat, or spawning logic:
...
### Por Quê This Matters for Security
...
### Por Quê Split Into Multiple Packages?
...
## Script Camada and Side Matrix

The 5-layer hierarchy (Capítulo 2.1) intersects with the server-client split. Not all layers run on all sides in the same way:

### Full Matrix

| Camada | Dedicated Server | Client | Listen Server | Notas |
|-------|:---:|:---:|:---:|-------|
...
Camadas 1 through 4 compile and run on **all sides**. The code is the same. This is why entity class definitions, config classes, and RPC constants all live in `3_Game` or `4_World` -- both sides need them.

Camada 5 (`5_Mission`) is where the split becomes explicit:
...
### Real Exemplo: Client Mission Hook in a Shared Mod
...
## Common Server-Client Padrãos

### Padrão 1: Server-Side Validation with Client Feedback
...
### Padrão 2: Config Sync (Server to Client)
...
**Correção:** If you need exclusive branches, use `else if` or check `IsDedicatedServer()`:
...
**Correção:** Use server/client specific subclasses or guard with context checks:
...
### requiredAddons[] Controla Load Order
...
## Real-World Split Exemplos

### Exemplo 1: StarDZ AI (Client + Server)
...
### Exemplo 3: StarDZ Missions (Client + Server)
...
### Erro 1: Running Server Logic on Client
...
### Erro 2: UI Code in Server-Only Mod
...
**Correção:** All UI code belongs in the client package (`type = "mod"`), wrapped in `#ifndef SERVER`.

### Erro 3: GetGame().GetPlayer() on Server
...
**Correção:** On the server, players are passed to you through events, RPCs, or iteration:
...
### Erro 4: Forgetting Listen Server Compatibility
...
### Erro 5: Not Using #ifdef for Optional Mod Detection
...
### Erro 6: Putting Shared Types Only in the Server Package
...
**Correção:** Shared data structures (RPC data, entity definitions, config classes) go in the client package (`type = "mod"`) so both sides have them:
...
### Erro 7: Hardcoded Server File Paths on Client
...
**Correção:** The server loads configs and sends relevant data to clients via RPC. Clients never read server config files directly.
...
**Anterior:** [Capítulo 2.5: File Organization Boas Práticas](05-file-organization.md)
**Próximo:** [Part 3: GUI & Layout System](../03-gui-system/01-widget-types.md)
